"""
 Devantsa RBI AI v3.0 PARALLEL PROCESSOR - BULL MARKET VERSION
Forked from Moon Dev's original research bot

 BULL MARKET SPECIALIZATION:
- Optimized for LONG-ONLY strategies on 2023-2024 bull market data
- Uses ideas_bull.txt for BULL-specific strategy patterns
- Target: 5-15 trades/year on 1h (TREND CAPTURE, not scalping)
- Benchmark: Buy-and-hold (BTC, ETH, SOL massive gains in bull)
- TRAILING STOPS ONLY - NO fixed take profits (let winners run!)
- Focus on momentum breakouts and trend continuation
...
- Uses ideas_bull.txt for BULL-specific strategies
...
7.  Multi-data results saved to ./results_bull/ folders for each strategy
...
TARGET_RETURN = 100  # Target return in % - agent will try to optimize to hit this (increased from 30% to push strategies harder)
...
COST_TRACKER_FILE = Path("src/data/rbi_devantsa/rbi_daily_cost.json")  # Track spending
...
# Update data directory paths - DEVANTSA Custom version
PROJECT_ROOT = Path(__file__).parent.parent
DATA_DIR = PROJECT_ROOT / "data/rbi_devantsa"
...
CURRENT_REGIME = None  # Market regime (BEAR/BULL/SIDEWAYS) - MUST BE SET! No default to prevent testing on wrong data
...
ENABLE_MULTI_ASSET_VALIDATION = True  # Toggle for Phase 3a
...
MULTI_ASSET_MAX_DD_THRESHOLD = {
    'GOLD': -20,    # No source can have worse than -20% DD
    'SILVER': -25,
    'BRONZE': -30
}
...
MULTI_ASSET_MIN_OVERALL_RETURN = {
    'GOLD': 5.0,    # Must have +5% or better average return across all 47 sources
    'SILVER': 2.0,  # Must have +2% or better average return
    'BRONZE': 0.0   # Must have positive average return (>0%)
}
...
ENABLE_CRYPTO_TIER_SYSTEM = True  # Set to False to disable crypto tiers

# Crypto assets list - ONLY these will be tested for crypto tiers
CRYPTO_ASSETS = [
    # Tier 1 (Must Pass) - Major liquid crypto
    "BTC-USD-15m",
    "ETH-USD-15m",
    "SOL-USD-15m",

    # Tier 2 (Should Pass) - Major crypto hourly
    "BTC-USD-1h",
    "ETH-USD-1h",
    "SOL-USD-1h",
    "ADA-USD-15m",
    ...
    # Tier 3 (Nice to Have) - Other liquid crypto
    "DOT-USD-15m",
    "LINK-USD-15m",
    "MATIC-USD-15m",
    "BNB-USD-15m",
...
CRYPTO_GOLD_THRESHOLD = {
    'min_positive_sources': 5,
    'min_avg_sharpe': 0.8,
    'max_avg_drawdown': -15.0,
    'min_avg_return': 8.0,
    'must_pass_btc': True,  # Must be positive on BTC-USD-15m
}

CRYPTO_SILVER_THRESHOLD = {
    'min_positive_sources': 2,  # Lowered from 3 (per-timeframe has fewer sources)
    'min_avg_sharpe': 0.5,      # Lowered from 0.6 (more lenient)
    'max_avg_drawdown': -20.0,   # Same
    'min_avg_return': 3.0,       # Lowered from 5.0 (more realistic for 4h data)
    'must_pass_major': True,     # Must pass at least one of BTC/ETH/SOL
}

CRYPTO_BRONZE_THRESHOLD = {
    'min_positive_sources': 2,   # Same (minimum viable)
    'min_avg_sharpe': 0.4,       # Lowered from 0.5
    'max_avg_drawdown': -25.0,    # Same
    'min_avg_return': 1.5,        # Lowered from 2.0
    'must_pass_major': True,      # Must pass at least one of BTC/ETH/SOL
}
...
CRYPTO_STATS_CSV = DATA_DIR / "backtest_stats_crypto.csv"  # Separate CSV for crypto
...
BACKTEST_PROMPT = """
Generate a LONG-ONLY strategy for bull market crypto trading based on the idea provided.

=== MANDATORY STOP VALUES ===
Initial stop: entry_price - (2.5 * ATR)
Trailing stop: highest_high - (2.0 * ATR)

=== FORBIDDEN PATTERNS (CAUSE SYNTAX ERRORS) ===
NEVER DO THESE - they cause line 56-76 syntax errors:
1. Trailing 'and' or 'or': if a > b and  <-- WRONG, incomplete
2. Missing colon: if condition  <-- WRONG, needs :
3. Unclosed parentheses: if (a > b <-- WRONG, missing )
4. Multiple statements on one line without semicolon
5. Using && or || instead of 'and'/'or'
6. Nested if without proper indentation
7. Comments inside conditions: if a > b # comment and c > d <-- WRONG

=== MANDATORY ENTRY PATTERN (USE EXACTLY) ===
# ALWAYS define conditions BEFORE combining:
cond1 = self.data.Close[-1] > self.indicator1[-1]
cond2 = self.indicator2[-1] > threshold
entry_signal = cond1 and cond2

# NEVER write this (causes syntax errors):
# entry_signal = self.data.Close[-1] > self.sma[-1] and self.rsi[-1] > 50 and
#                self.momentum[-1] > 0  <-- MULTILINE = SYNTAX ERROR

=== DO NOT MODIFY TEMPLATE SECTIONS ===
The elif self.position.is_long: block and trailing stop logic MUST stay exactly as provided.
Only modify: indicators in init() and entry_signal calculation.

=== API RULES ===
- Use self.I() for ALL indicators
- Track entry_price, trailing_stop, highest_since_entry as class variables
- NO talib, NO self.position.entry_price, NO _broker methods
...
class MyStrategy(Strategy):
    ...
    def next(self):
        if not self.position:
            # ENTRY LOGIC - replace this line with your conditions
            entry_signal = self.data.Close[-1] > self.sma[-1]
            if entry_signal:
                self.entry_price = self.data.Close[-1]
                self.highest_since_entry = self.data.High[-1]
                self.trailing_stop = self.entry_price - (2.5 * self.atr[-1])
                self.buy()

        elif self.position.is_long:
            if self.data.High[-1] > self.highest_since_entry:
                self.highest_since_entry = self.data.High[-1]
                new_stop = self.highest_since_entry - (2.0 * self.atr[-1])
...
Replace the entry_signal line with your implementation. Keep stops at 2.5 and 2.0.
NO emojis. Raw strings for paths. OUTPUT ONLY VALID PYTHON CODE.
IMPORTANT: Use 4h timeframe data (BTC-USD-4h.csv) - do NOT use 15m or 1h as default.
"""


OPTIMIZE_PROMPT = """
Optimize this bull market LONG strategy for better performance.

Current Return: {current_return}%
Target: {target_return}%

=== DO NOT CHANGE (MANDATORY) ===
- Initial stop: entry_price - (2.5 * ATR) -- KEEP EXACTLY 2.5
- Trailing stop: highest_high - (2.0 * ATR) -- KEEP EXACTLY 2.0
- NEVER change stop multipliers to 3.0x, 3.5x, or higher

=== OPTIMIZATION FOCUS ===
1. Entry timing - catch moves earlier or with better confirmation
2. Add more entry signals (OR logic) to increase trade count
3. Position sizing improvements
4. Filter out bad setups

=== RULES ===
- NO emojis (Windows crash)
- Raw strings for paths: r'c:\\Users\\...'
- Use self.I() for indicators
- Keep trailing stop logic intact
...
DEBUG_PROMPT = """
Fix technical issues in the backtest code WITHOUT changing strategy logic.

=== MANDATORY STOP VALUES (NEVER CHANGE) ===
- Initial stop: entry_price - (2.5 * ATR) -- MUST be 2.5
- Trailing stop: highest_high - (2.0 * ATR) -- MUST be 2.0
- If you see 3.0, 3.5, 3.8 etc., change them to 2.5/2.0

=== NO EMOJIS (Windows crash) ===
Remove all emojis from print statements.

=== SYNTAX ERRORS (LINE 56-76 TYPE ISSUES) ===
If you see "SyntaxError: invalid syntax" at line X, check for:
1. Missing colon after if/elif/else/for/while/def/class
2. Unclosed parentheses - count ( and ) they must match
3. Trailing 'and'/'or' with no condition after it
4. Using && or || instead of 'and'/'or'
5. Missing closing brackets [ ] or braces {{ }}
6. Multi-line conditions without proper continuation
7. Comments inside condition statements

MANDATORY FIX PATTERN for entry logic:
```python
# WRONG: These ALL cause syntax errors at line 56-65
entry_signal = self.close[-1] > self.sma[-1] and self.rsi[-1] > 50 and  # trailing and
entry_signal = self.data.Close[-1] > self.sma[-1] and
               self.momentum[-1] > 0  # multiline = BROKEN
if (self.data.Close[-1] > self.sma[-1]  # unclosed paren

# RIGHT: ALWAYS use separate condition variables
cond1 = self.data.Close[-1] > self.sma[-1]
cond2 = self.rsi[-1] > 50
entry_signal = cond1 and cond2
if entry_signal:
    # trade logic here
...
WHEN FIXING SYNTAX ERRORS:
1. Find the entry_signal line and surrounding code
2. Split ALL conditions into separate cond1, cond2, etc. variables
3. Combine with: entry_signal = cond1 and cond2
4. Ensure all if/elif have colons
5. Count parentheses - ( must equal )
...
INDICATOR LOGIC OVERRIDE (BEAR):
- USE ABSOLUTE THRESHOLDS: RSI < 28 for oversold, RSI > 65-75 for overbought
- AVOID PERCENTILE RANKING: Percentile ranking assumes positive drift (doesn't exist in bear)
- MEAN REVERSION WORKS IN BEAR: Capitulation bounces are THE primary edge
- NEGATIVE MOMENTUM FILTERS: Look for momentum < -0.045 to -0.075 (extreme selling pressure)
- VOLUME PATTERNS: Spike > 2.0-2.5x avg on capitulation, decline < 0.8-1.0x on weak rallies

ENTRY LOGIC OVERRIDE (BEAR):

FOR LONG ENTRIES (40% of strategies - quick capitulation bounces):
- RSI(14) < 25-33 (extreme oversold - this IS mean reversion!)
- Volume spike > 2.0-2.5x avg (panic selling exhaustion)
- Momentum < -0.055 to -0.075 (capitulation move)
- Price < SMA(50) * 0.88-0.92 (extreme deviation from mean)
- Entry: When price shows first sign of reversal (hammer, bullish engulfing, volume dry-up)

FOR SHORT ENTRIES (60% of strategies - fade failed rallies):
- RSI(14) > 65-75 (overbought in bear market context)
- Volume DECLINING on rally < 0.8-1.0x avg (weak rally, no conviction)
- Price rejection at resistance (SMA(20), SMA(50), SMA(100), prior highs)
- Momentum weak < 0.02-0.03 (rally can't sustain, exhausting)
- Breakdown continuation: Support breaks, retest fails with declining volume

EXIT LOGIC OVERRIDE (BEAR):
- LONG exits: Take profit FAST at 3.5-4.6x stop distance (bear resumes quickly)
- SHORT exits: Let winners run to 4.0-6.0x stop distance (downtrends persist)
- LONG max hold: 20-32 bars (bounces are brief, exit before bear resumes)
- SHORT max hold: 24-48 bars (ride downtrends to next support level)
- Use FIXED TP targets (not trailing stops) for both LONG and SHORT

RISK MANAGEMENT OVERRIDE (BEAR):
- Position sizing: 0.5-0.75% risk per trade (more conservative in volatile bear)
- LONG stops: 2.8-3.5x ATR (need room for volatility on capitulation)
- SHORT stops: 2.5-3.2x ATR (tighter, bear rallies are violent but brief)
- Risk/Reward: LONG = 3.5-4.6x, SHORT = 4.0-6.0x (favor shorts)

BANNED PATTERNS IN BEAR MARKETS:
- Donchian breakouts to upside (false breakouts fail in sustained downtrends)
- Trend continuation strategies for LONG (no sustained uptrends in bear)
- High-frequency momentum strategies (200-400 trades causes commission death)
- Percentile ranking systems (assume positive drift that doesn't exist)
- Long holding periods >50 bars (bear bounces are brief)

EXAMPLE BEAR STRATEGY STRUCTURE:
```python
def next(self):
    # LONG Example - Capitulation Bounce
    rsi = self.rsi[-1]
    momentum = (self.data.Close[-1] / self.data.Close[-8] - 1)
    vol_ratio = self.data.Volume[-1] / self.vol_avg[-1]
    sma50 = self.sma50[-1]

    if (rsi < 28 and                           # Extreme oversold (absolute threshold!)
        momentum < -0.058 and                  # Capitulation move
        vol_ratio > 2.2 and                    # Panic volume
        self.data.Close[-1] < sma50 * 0.90):  # Extreme deviation

        atr = self.atr[-1]
        stop_loss = self.data.Close[-1] - (2.9 * atr)
        take_profit = self.data.Close[-1] + (4.3 * (self.data.Close[-1] - stop_loss))
        size = 0.6  # 0.6% risk
        self.buy(size=size, sl=stop_loss, tp=take_profit)

    # Exit after 25 bars (bounce exhausted)
    if self.position and len(self.data) - self.trades[-1].entry_bar >= 25:
        self.position.close()
```

Remember: BEAR strategies need BALANCE (52-104 trades = 1-2 per week), not starvation (1 trade) or overtrading (200+ trades)!
"""
...
REGIME_OVERRIDES = {
    ...
    'BULL_FULL': BULL_REGIME_OVERRIDES,  # BULL_FULL uses same rules as BULL (both bull markets)
...
def validate_regime_compliance(stats: dict, thread_id: int) -> tuple:
    ...
    elif CURRENT_REGIME == 'BULL_FULL':
        # BULL_FULL is 6-month period, need 1-4 trades per week = 26-104 trades total
        # 4h timeframe: ~1080 bars, need signal every 10-42 bars
        if trades > 130:
            return False, f"BULL_FULL regime expects 26-104 trades (1-4/week), got {trades} (over-trading!)"
        elif trades < 20:
            return False, f"BULL_FULL regime expects 26-104 trades (1-4/week), got {trades} (filters too tight!)"
...
def parse_and_log_multi_data_results(strategy_name: str, thread_id: int, backtest_file_path: Path) -> None:
    """
     Phase 3a: Multi-Asset Results Parser with Tier Classification!
    Parses multi-data testing results and classifies strategy using multi-asset tier logic
    ...
    """
    try:
        # Multi-data results are saved in ./results_bull/ relative to PROJECT ROOT (where subprocess.run executes)
        # NOT relative to the backtest file location
        project_root = Path(__file__).parent.parent.parent  # Go up from src/agents/ to project root
        results_dir = project_root / "results_bull"
        ...
        # PHASE 3A: Classify using multi-asset tier logic
        if ENABLE_MULTI_ASSET_VALIDATION:
            tier = classify_multi_asset_tier(df, thread_id)

            if tier == "REJECT":
                thread_print(" Strategy rejected - did not meet multi-asset tier requirements", thread_id, "red")
                return

            thread_print(f" Multi-asset tier: {tier}", thread_id, "green", attrs=['bold'])

            # Log ALL results for this tier-qualified strategy (not just passing threshold)
            # This gives full visibility into performance across all assets
            for idx, row in df.iterrows():
                ...
                # Log to CSV with tier classification
                log_stats_to_csv(
                    strategy_name,
                    thread_id,
                    stats,
            ...
            thread_print(f" Logged {len(df)} multi-asset results with {tier} tier!", thread_id, "green", attrs=['bold'])

        # NEW: Crypto-only tier classification (runs in parallel, doesn't affect traditional tiers)
        if ENABLE_CRYPTO_TIER_SYSTEM:
            crypto_tier, crypto_color, crypto_stats = classify_crypto_tier(df, thread_id)

            if crypto_tier:  # Only log if crypto tier qualified
                thread_print(f" {crypto_tier} tier qualified!", thread_id, crypto_color, attrs=['bold'])

                # Filter to crypto results only
                crypto_df = df[df['Data_Source'].isin(CRYPTO_ASSETS)]

                # Log ALL crypto results for this crypto-tier-qualified strategy
                for idx, row in crypto_df.iterrows():
                    ...
                    # Log to crypto CSV with crypto tier classification
                    log_crypto_stats_to_csv(
                        strategy_name,
                        thread_id,
                        stats,
                        str(backtest_file_path),
                ...
                thread_print(f" Logged {len(crypto_df)} crypto results with {crypto_tier} tier!", thread_id, crypto_color, attrs=['bold'])

        else:
            # FALLBACK: Old behavior if multi-asset validation is disabled
...
def force_strategy_name(code: str, strategy_name: str) -> str:
    """
    Force the correct strategy name in generated code to prevent CSV overwrites.

    BUG FIX: AI sometimes keeps generic 'MyStrategy' class name instead of using
    the actual strategy name. This causes multiple strategies to save results
    to 'MyStrategy.csv', overwriting each other's data.

    This function replaces all occurrences of 'MyStrategy' with the actual
    strategy name extracted during research phase.

    Args:
        code: The generated backtest code
        strategy_name: The correct strategy name to use

    Returns:
        Code with MyStrategy replaced by actual strategy name
    """
    # Skip if strategy_name is already MyStrategy or invalid
    if not strategy_name or strategy_name == "MyStrategy" or strategy_name == "UnknownStrategy":
        return code

    # Replace class definition: class MyStrategy(Strategy) -> class ActualName(Strategy)
    code = re.sub(r'class\s+MyStrategy\s*\(', f'class {strategy_name}(', code)

    # Replace Backtest instantiation: Backtest(data, MyStrategy -> Backtest(data, ActualName
    code = re.sub(r'Backtest\s*\(\s*data\s*,\s*MyStrategy', f'Backtest(data, {strategy_name}', code)

    # Replace test_on_all_data call: test_on_all_data(MyStrategy, 'MyStrategy' -> test_on_all_data(ActualName, 'ActualName'
    code = re.sub(r'test_on_all_data\s*\(\s*MyStrategy', f'test_on_all_data({strategy_name}', code)
    code = re.sub(r"'MyStrategy'", f"'{strategy_name}'", code)
    code = re.sub(r'"MyStrategy"', f'"{strategy_name}"', code)
...
def health_check_code(code: str, strategy_name: str, thread_id: int) -> tuple[bool, str, str]:
    ...
    try:
        ...
        # Check 4b: Detect trailing operators (common syntax error cause)
        # Pattern: line ends with 'and', 'or', '+', '-', '*', '/', ',' without continuation
        trailing_op_patterns = [
            r'\band\s*$',   # Line ends with 'and'
            r'\bor\s*$',    # Line ends with 'or'
            r'\band\s*#',   # 'and' followed by comment
            r'\bor\s*#',    # 'or' followed by comment
        ]
        for line_num, line in enumerate(code.split('\n'), 1):
            stripped = line.rstrip()
            for pattern in trailing_op_patterns:
                if re.search(pattern, stripped):
                    return False, f"Trailing operator at line {line_num}: '{stripped[-20:]}' - conditions must be on same line or use separate variables", code

        # Check 4c: Detect unclosed parentheses in conditions
        for line_num, line in enumerate(code.split('\n'), 1):
            if 'if ' in line or 'elif ' in line or 'entry_signal' in line:
                open_parens = line.count('(') - line.count(')')
                if open_parens > 0 and not line.rstrip().endswith('\\'):
                    return False, f"Unclosed parenthesis at line {line_num}: '{line.strip()[:50]}' - count your ( and )", code

        # Check 5: REMOVED - compile() syntax check was too aggressive
        # It rejected valid code that would run fine in execution
        # Let the actual backtest execution catch real syntax errors instead
        ...
        if '_broker.getvalue()' in code:
            return False, "Uses _broker.getvalue() (incorrect API - use self.equity instead)", code
...
def save_backtest_if_threshold_met(code: str, stats: dict, strategy_name: str, iteration: int, thread_id: int, phase: str = "debug") -> bool:
    """
    Tier-based Strategy Saver - Only Gold/Silver/Bronze strategies are saved!
    ...
    Returns:
        True if saved (tier qualified), False if rejected
    """
    return_pct = stats.get('return_pct')

    # Classify strategy into tier (single-source tier for backward compatibility)
    tier = classify_strategy_tier(stats)

    # NEW: Also check crypto tier qualification (multi-source, best-timeframe evaluation)
    crypto_tier = None
    crypto_tier_qualified = False
    if ENABLE_CRYPTO_TIER_SYSTEM:
        # Check if multi-data results exist
        project_root = Path(__file__).parent.parent.parent
        results_dir = project_root / "results_bull"
        results_csv = results_dir / f"{strategy_name}.csv"
        ...
        if results_csv.exists():
            try:
                ...
                if crypto_tier:
                    crypto_tier_qualified = True
                    # Save to crypto-tier-specific folders
                    save_to_crypto_tier_folder(code, strategy_name, iteration, thread_id, phase, crypto_tier, crypto_stats, crypto_color)
            except Exception as e:
                thread_print(f" Error checking crypto tier: {str(e)}", thread_id, "yellow")

    # REJECT strategies that don't meet any tier requirements (neither single-source nor crypto)
    if tier == "REJECT" and not crypto_tier_qualified:
    ...
    # If only crypto tier qualified (not single-source), skip traditional tier saving
    if tier == "REJECT" and crypto_tier_qualified:
        thread_print(f" Only crypto tier qualified - skipping traditional tier save", thread_id, "cyan")
        return True

    try:
        # Determine filename based on phase and tier
        if phase == "debug":
            filename = f"T{thread_id:02d}_{strategy_name}_{tier}_DEBUG_v{iteration}_{return_pct:.1f}pct.py"
        elif phase == "opt":
            filename = f"T{thread_id:02d}_{strategy_name}_{tier}_OPT_v{iteration}_{return_pct:.1f}pct.py"
        else:  # final
            filename = f"T{thread_id:02d}_{strategy_name}_{tier}_FINAL_{return_pct:.1f}pct.py"

        # Select tier-specific directory
        if tier == "GOLD":
            tier_dir = GOLD_STRATEGIES_DIR
            tier_color = "green"
        elif tier == "SILVER":
            tier_dir = SILVER_STRATEGIES_DIR
            tier_color = "cyan"
        else:  # BRONZE
            tier_dir = BRONZE_STRATEGIES_DIR
            tier_color = "yellow"

        # Save to TIER-SPECIFIC folder (primary storage)
        tier_file = tier_dir / filename
        with file_lock:
            with open(tier_file, 'w', encoding='utf-8') as f:
                f.write(code)
        ...
        # Save to DEVANTSA_WINNERS folder (central repository for all winners across all dates)
        winners_file = DEVANTSA_WINNERS_DIR / filename
        with file_lock:
            with open(winners_file, 'w', encoding='utf-8') as f:
                f.write(code)

        thread_print(f" {tier} TIER! Saved! Return: {return_pct:.2f}%", thread_id, tier_color, attrs=['bold'])

        # Log to CSV with tier information
        log_stats_to_csv(strategy_name, thread_id, stats, str(tier_file), tier=tier)

        #  Generate comprehensive strategy report for winning strategies
...
"""
    ...
    if output:
        ...
        # BUG FIX: Force correct strategy name to prevent CSV overwrites
        output = force_strategy_name(output, strategy_name)
            ...
            if execution_result['success']:
                if has_nan_results(execution_result):
                    ...
                    if current_return >= TARGET_RETURN:
                        ...
                        while optimization_iteration < MAX_OPTIMIZATION_ITERATIONS:
                            ...
                            if is_improvement:
                                ...
                                if new_return >= TARGET_RETURN:
                                    ...
                                    # BUG FIX: Ensure correct class name before saving
                                    best_code = force_strategy_name(best_code, strategy_name)
                                    with file_lock:
                                        with open(final_file, 'w', encoding='utf-8') as f:
                                            f.write(best_code)
                        ...
                        # BUG FIX: Ensure correct class name before saving BEST file
                        best_code = force_strategy_name(best_code, strategy_name)
                        with file_lock:
...
def main(ideas_file_path=None, run_name=None, regime=None):
    ...
    BULL MARKET VERSION: Defaults to BULL regime if not specified.
    """
    ...
    # BULL MARKET VERSION: Default to BULL if not specified
    if regime is None:
        regime = 'BULL_FULL'
        print("\n[INFO] No regime specified, defaulting to BULL_FULL (Oct 2023 - March 2024, SOL 900%)")

    # Validate regime (allow BULL_FULL by default for this bull-specific agent)
    if regime not in ['BEAR', 'BULL', 'BULL_FULL', 'SIDEWAYS']:
        raise ValueError(
            f"\n{'='*80}\n"
            f"ERROR: Invalid regime specified!\n"
            f"This is the BULL MARKET agent - defaults to BULL_FULL regime.\n"
            f"Use: --regime BULL_FULL (default) | --regime BULL | --regime BEAR | --regime SIDEWAYS\n"
            f"\n"
            f"BULL_FULL = Oct 2023 - March 2024 (SOL 900%, BTC 160%)\n"
            f"BULL = Q1 2024 only (SOL 91%, BTC 65%)\n"
            f"\n"
            f"Example:\n"
            f"  python src/agents/rbi_agent_pp_multi_devantsa_bull.py\n"
            f"  python src/agents/rbi_agent_pp_multi_devantsa_bull.py --regime BULL_FULL\n"
            f"{'='*80}\n"
        )

    # Store regime globally for backtest code generation
    CURRENT_REGIME = regime

    # CRITICAL FIX: Set regime-specific data directory
    if CURRENT_REGIME == 'BEAR':
        MULTI_DATA_DIR = PROJECT_ROOT / "data" / "rbi_regime_bear"
        cprint(f"\n  Using BEAR regime data: {MULTI_DATA_DIR}", "yellow", attrs=['bold'])
    elif CURRENT_REGIME == 'BULL':
        MULTI_DATA_DIR = PROJECT_ROOT / "data" / "rbi_regime_bull"
        cprint(f"\n  Using BULL regime data (Q1 2024 only): {MULTI_DATA_DIR}", "yellow", attrs=['bold'])
    elif CURRENT_REGIME == 'BULL_FULL':
        MULTI_DATA_DIR = PROJECT_ROOT / "data" / "rbi_regime_bull_full"
        cprint(f"\n  Using FULL BULL regime data (Oct 2023 - March 2024, SOL 900%): {MULTI_DATA_DIR}", "green", attrs=['bold'])
    elif CURRENT_REGIME == 'SIDEWAYS':
        MULTI_DATA_DIR = PROJECT_ROOT / "data" / "rbi_regime_sideways"
        cprint(f"\n  Using SIDEWAYS regime data: {MULTI_DATA_DIR}", "yellow", attrs=['bold'])
    else:
        MULTI_DATA_DIR = PROJECT_ROOT / "data" / "rbi_multi"  # Fallback to mixed data
        cprint(f"\n  Using mixed regime data: {MULTI_DATA_DIR}", "red", attrs=['bold'])
    ...
    if ideas_file_path:
        ...
        # ALWAYS use regime-specific ideas file
        # NOTE: BULL and BULL_FULL share the same ideas file (ideas_bull.txt)
        if regime in ['BULL', 'BULL_FULL']:
            IDEAS_FILE = DATA_DIR / "ideas_bull.txt"
        else:
            IDEAS_FILE = DATA_DIR / f"ideas_{regime.lower()}.txt"
    ...
    cprint(f" Market Regime: {regime}", "green", attrs=['bold'])
    if run_name:
...
if __name__ == "__main__":
    ...
    # BULL MARKET VERSION - defaults to BULL_FULL regime (Oct 2023 - March 2024, SOL 900%, BTC 160%)
    parser = argparse.ArgumentParser(description="Devantsa RBI Agent - BULL Market Parallel Backtest Processor")
    parser.add_argument('--ideas-file', type=str, help='Custom ideas file path (overrides default ideas_bull.txt)')
    parser.add_argument('--run-name', type=str, help='Run name for folder organization')
    parser.add_argument('--regime', type=str, choices=['BEAR', 'BULL', 'BULL_FULL', 'SIDEWAYS'], default='BULL_FULL',
                        help='Market regime for strategy testing (default: BULL_FULL = Oct 2023 - March 2024 with SOL 900%% gains)')
