"""
 Devantsa RBI AI v3.0 PARALLEL PROCESSOR - BEAR MARKET VERSION
Forked from Moon Dev's original research bot

 BEAR MARKET SPECIALIZATION:
- Optimized for SHORT-ONLY strategies on 2022 bear market data
- Uses ideas_bear.txt for BEAR-specific strategy patterns
- Target: 52-120 trades/year on 1h (1-2 trades per week minimum)
- Benchmark: SHORT-and-hold (BTC +64.2%, ETH +67.5%, SOL +94.2%)
- NO percentile ranking - uses absolute thresholds (RSI > 28-34)
...
- Uses ideas_bear.txt for BEAR-specific strategies
...
TARGET_RETURN = 45  # Target return in % - agent will try to optimize to hit this (increased from 30% to push strategies harder)
    ...
    def next(self):
        if not self.position:
            if <entry_signal>:
                ...
                stop_dist = 2.7 * self.atr[-1]
                self.tp1_price = self.entry_price - (4.0 * stop_dist)
                self.tp2_price = self.entry_price - (7.0 * stop_dist)
            ...
            if not self.tier1_hit and self.data.Close[-1] <= self.tp1_price:
                self.position.close(0.5)  # CORRECT: close 50% of position
                self.tier1_hit = True
            elif self.data.Close[-1] <= self.tp2_price:
                self.position.close()  # Close remaining 50%
            elif self.bars_in_trade >= 85:
                self.position.close()  # Max hold exit

# ❌ WRONG WAY to close partial position:
# half_size = abs(self.position.size) * 0.5
# self.buy(size=half_size)  # DO NOT DO THIS!

# ✅ CORRECT WAY:
# self.position.close(0.5)  # Closes 50% automatically
```
...
ROLE:
- Generate backtesting.py compatible code for SHORT-focused strategies
- Target 2022 bear market data (Dec 31, 2021 → Dec 30, 2022)
- Benchmark: SHORT-and-hold (BTC +64.2%, ETH +67.5%, SOL +94.2%)
- Goal: 52-120 trades on 1h timeframe, 40-60% of benchmark return
...
class MyStrategy(Strategy):
    def init(self):
        ...
        self.stop_loss = None    # Track manually
        self.tp1_price = None    # Track manually
        self.tp2_price = None    # Track manually
        self.bars_in_trade = 0   # Track manually
        self.tier1_hit = False   # Track manually

        # Indicators here...

    def next(self):
        # Entry - calculate TP prices HERE
        if not self.position:
            if <entry_condition>:
                self.entry_price = self.data.Close[-1]
                stop_dist = 2.7 * self.atr[-1]
                self.tp1_price = self.entry_price - (4.0 * stop_dist)  # SHORT
                self.tp2_price = self.entry_price - (7.0 * stop_dist)  # SHORT

                self.sell(sl=self.entry_price + stop_dist)  # Pass sl to sell()
                self.bars_in_trade = 0
                self.tier1_hit = False

        # Exit - check TP manually
        elif self.position.is_short:
            self.bars_in_trade += 1
            ...
            if not self.tier1_hit and self.data.Close[-1] <= self.tp1_price:
                ...
                self.position.close()  # Close all at tier 2
            elif self.bars_in_trade >= 85:
                self.position.close()  # Max hold exit
```
...
ENTRY LOGIC:
- EXACTLY 2 conditions (3+ causes signal starvation)
- Each condition 8-12% selective
- Expected trades: 52-120 on 1h timeframe

APPROVED PATTERNS:
1. RSI + Breakdown: RSI > 28-34 AND Close < Low_N.shift(1)
2. RSI + Momentum: RSI > 28-34 AND Momentum < -0.025 to -0.032
3. RSI + Volume: RSI > 28-34 AND Volume > 1.5-1.8x avg
4. Breakdown + Volume: Close < Low_N AND Volume > 1.5-1.8x avg
5. Momentum + Volume: Momentum < -0.025 AND Volume > 1.5-1.8x avg

BANNED:
- .rolling().rank(pct=True) percentile ranking
- 3+ entry conditions
- Trailing stops (use fixed 2-tier TP)

RISK MANAGEMENT (MANUAL TRACKING REQUIRED):
- Stop: 2.7x ATR(14) for SHORTS (pass to sell(sl=...) call)
- TP Tier 1: 4.0x stop distance (close 50%) - track as self.tp1_price
- TP Tier 2: 7.0x stop distance (close remaining 50%) - track as self.tp2_price
- Max hold: 80-90 bars for SHORTS - track as self.bars_in_trade
- Risk: 0.65% per trade
- REMEMBER: Calculate entry_price, tp1_price, tp2_price in next() when entering trade!

INDICATOR THRESHOLDS:
- RSI: > 25-35 for SHORTS (most common: 28-32)
- Momentum_7: < -0.020 to -0.035 (most common: -0.025 to -0.032)
- Volume: > 1.3-1.8x rolling_avg (most common: 1.5-1.7x)
- Low_N: N=10-20 bars
- ATR: period=14
...
class BearShortStrategy(Strategy):
    def init(self):
        ...
        # Indicators (choose 2 for entry)
        self.rsi = self.I(RSI, self.data.Close, 14)
        self.low_14 = self.I(lambda x: pd.Series(x).rolling(14).min(), self.data.Low)
        self.momentum_7 = self.I(lambda c: pd.Series(c).pct_change(7), self.data.Close)
        self.vol_avg = self.I(lambda v: pd.Series(v).rolling(20).mean(), self.data.Volume)
        self.atr = self.I(calculate_atr, self.data.High, self.data.Low, self.data.Close, 14)

    def next(self):
        # Entry (EXACTLY 2 conditions - example: RSI + Volume)
        if not self.position:
            if self.rsi[-1] > 30 and self.data.Volume[-1] > 1.75 * self.vol_avg[-1]:
                self.entry_price = self.data.Close[-1]
                stop_dist = 2.7 * self.atr[-1]
                self.tp1_price = self.entry_price - (4.0 * stop_dist)
                self.tp2_price = self.entry_price - (7.0 * stop_dist)
...
if __name__ == "__main__":
    ...
    bt = Backtest(data, BearShortStrategy, cash=1_000_000, commission=0.002)
    stats = bt.run()
    ...
    print("="*80)
    print("MULTI-DATA BACKTEST - Testing on 25+ Data Sources")
    print("="*80)
...
✓ 2-tier fixed TP (4.0x and 7.0x)
✓ Stop: 2.7x ATR for SHORTS
✓ Max hold: 80-90 bars
✓ Risk: 0.65%
...
def parse_and_log_multi_data_results(strategy_name: str, thread_id: int, backtest_file_path: Path) -> None:
    ...
    """
    try:
        # Multi-data results are saved in ./results/ relative to PROJECT ROOT (where subprocess.run executes)
...
def main(ideas_file_path=None, run_name=None, regime=None):
    ...
    # CRITICAL VALIDATION: Regime MUST be specified!
    if regime not in ['BEAR', 'BULL', 'SIDEWAYS']:
        raise ValueError(
            f"\n{'='*80}\n"
            f"ERROR: You MUST specify a market regime!\n"
            f"Use: --regime BEAR | --regime BULL | --regime SIDEWAYS\n"
            f"\n"
            f"Examples:\n"
            f"  python src/agents/rbi_agent_pp_multi_devantsa.py --regime BEAR\n"
            f"  python src/agents/rbi_agent_pp_multi_devantsa.py --regime BULL\n"
            f"  python src/agents/rbi_agent_pp_multi_devantsa.py --regime SIDEWAYS\n"
            f"\n"
            f"Or use the batch files:\n"
            f"  run_bear_strategies.bat\n"
            f"  run_bull_strategies.bat\n"
            f"  run_sideways_strategies.bat\n"
            f"{'='*80}\n"
        )
    ...
    if CURRENT_REGIME == 'BEAR':
        ...
        cprint(f"\n  Using BULL regime data: {MULTI_DATA_DIR}", "yellow", attrs=['bold'])
    elif CURRENT_REGIME == 'SIDEWAYS':
