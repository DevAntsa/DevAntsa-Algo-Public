...
TARGET_RETURN = 100  # Batch 14: RETURN MAXIMIZER. Push past DirectionalBreakout +76.7%. All strategies MUST pyramid.
...
MAX_OPTIMIZATION_ITERATIONS = 6  # v6+ proven to degrade strategies. Saves 40% API spend
...
IDEAS_FILE = RBI_REGULAR_DIR / "ideas.txt"
CURRENT_REGIME = None
...
def update_date_folders():
    ...
    with date_lock:
        ...
        WINNERS_DIR = RBI_REGULAR_DIR / "winners"
...
========================================================================
 FULL-DATA SELF-GATING PRINCIPLE
========================================================================
Strategies are tested on ALL market data (bull + crash + bear + recovery + sideways, ~5 years).
Strict entry conditions simply don't trigger in wrong regimes - the entry conditions ARE
the regime filter. No regime detector needed.

KEY EVIDENCE: 128 strategies tested on multi-regime data ALL survived bear with <6.5% loss.
One was even profitable (+0.70%) during SOL's -94% crash. Strategies with 1-2 loose conditions
failed catastrophically in live trading (lost 14-95% in 2022 bear).

The ONE long that survived had 3 strict entry filters + 0.5% position sizing.

========================================================================
 MANDATORY ENTRY CONDITION RULES
========================================================================
MINIMUM 3 independent entry conditions. Strategies with 1-2 conditions fail in bear markets.
Adding a 4th or 5th condition (especially stacked EMA) is STRONGLY RECOMMENDED for higher Sharpe.

At least one MUST be a momentum/trend filter (prevents firing in wrong regime):
- Price > SMA(200): macro trend gate
- EMA(short) > EMA(medium) > EMA(long): stacked EMA (STRONGLY RECOMMENDED as bonus filter)
- Momentum(10) > 6% threshold: momentum gate
- ADX > 25-30: trend strength gate
- ROC percentile > 0.70: relative momentum gate

Volume confirmation (boolean, not percentile) counts as one condition:
- Volume > 1.5-2.0x rolling 20-bar average

Consider RSI(14) between 55-75 as a quality filter (bullish but not overbought).

CORRECT (3+ conditions, self-gating):
```python
trend_ok = self.data.Close[-1] > self.sma200[-1]              # Condition 1: macro trend
breakout = self.data.Close[-1] > self.high_24[-1]             # Condition 2: breakout
volume = self.data.Volume[-1] > 1.8 * self.vol_avg[-1]        # Condition 3: volume
# BONUS (strongly recommended for higher Sharpe):
# ema_stack = self.ema8[-1] > self.ema21[-1] > self.ema50[-1]  # Condition 4: stacked EMA

if trend_ok and breakout and volume and not self.position:
    self.buy(size=size)
```

WRONG (1-2 conditions, no self-gating):
```python
if self.rsi[-1] < 30:  # Single condition = fires constantly in bears
    self.buy()
```

========================================================================
 PRE-BAKED TREND FILTER (MANDATORY)
========================================================================
EVERY strategy MUST include at least ONE of these trend filters as an entry condition:
1. Close > SMA(200) - macro trend gate (RECOMMENDED)
2. Stacked EMA check - e.g. EMA(8) > EMA(21) > EMA(50) (STRONGLY RECOMMENDED as bonus 4th condition)
3. ADX > 25 - trend strength filter

Using BOTH a long-term filter AND stacked EMA is ideal for higher Sharpe,
but one trend filter is the minimum requirement.

CORRECT (trend filter + stacked EMA bonus):
```python
macro_ok = self.data.Close[-1] > self.sma200[-1]         # MANDATORY: at least one trend filter
ema_stack = self.ema8[-1] > self.ema21[-1] > self.ema50[-1]  # BONUS: stacked EMA for higher Sharpe
breakout_ok = self.data.Close[-1] > self.high_25[-1]      # Condition 3
volume_ok = self.data.Volume[-1] > 1.7 * vol_avg[-1]      # Condition 4

if macro_ok and ema_stack and breakout_ok and volume_ok:
    self.buy(size=size)
```

WRONG (no trend filter at all):
```python
if self.rsi[-1] > 50 and self.momentum[-1] > 0.04:
    self.buy(size=size)  # Will trade in bear markets until optimizer fixes it
```

========================================================================
 TRADE FREQUENCY TARGETS (4H DATA, 5 YEARS)
========================================================================
System uses 4h data ONLY (BTC/ETH/SOL 4h, ~5 years from Jan 2021).
With trailing stops, each trade is held longer = fewer total trades needed.

- BTC-4h: 30-260 trades (ideal: 80-180 for return maximization, 30-80 for Sharpe focus)
  B14 DIBreakoutPyramid has 156 trades at S=1.23 (BEST EVER). Higher trade count is FINE if Sharpe holds.
- SOL/ETH-4h: 50-300 trades (ideal: 100-200)
- Strategies with 300+ trades on 4h are CHURNING
- Strategies with <20 trades are statistically meaningless

Commission drag (0.1% round-trip, Bybit futures):
- 60 trades = 6% drag (sweet spot)
- 150 trades = 15% drag (acceptable with high expectancy like B14)
- 250 trades = 25% drag (upper limit, needs high win rate)
...
INDICATOR PERIODS (4H DATA):
- 4h bars: ATR(14) = 56 hours (~2.3 days). SMA(200) = ~33 days. SMA(50) = ~8 days.
- Use periods 5-30 bars for responsive signals on 4h.
- BTC: MIXED lookbacks still recommended - one FAST (5-14 bars) + one SLOW (20-50 bars)
- SOL/ETH: Moderate periods (10-30 bars) work well on 4h.
...
MODE 3 - PYRAMID + ADAPTIVE TRAIL (DEFAULT for all new strategies):
Stacks up to 3 units on winning positions. Trail tightens as profit grows.
Each unit risks 1% (2% total max exposure with MAX_UNITS=2). All units close at once on trail hit.
4H NOTE: ATR(14) = 56 hours (~2.3 days). SMA(200) = ~33 days. SMA(50) = ~8 days. Use periods 5-30 for responsive signals.
```python
class MyStrategy(Strategy):
    MAX_UNITS = 2  # Proven: 2 > 3 for Sharpe (B14 DIBreakoutPyramid S=1.23)
    def init(self):
    ...
    def next(self):
        ...
        if can_enter:
            trend_ok = self.data.Close[-1] > self.sma200[-1]
            entry_signal = trend_ok and <cond2> and <cond3>  # Volume is OPTIONAL for BTC
            if entry_signal:
                entry_price = self.data.Close[-1]
                stop_dist = 2.5 * atr  # Proven: 2.5x > 3.0x/3.5x for Sharpe (Batch 11)
                raw_size = 0.005 / (stop_dist / entry_price)  # 0.5% risk per unit (proven optimal for SOL, B16 sweep: S=1.48)
        ...
        # SIGNAL EXITS (CRITICAL - boosts Sharpe by 0.1-0.3 vs trail-only!)
        # Reverse your entry conditions to detect when the setup is DEAD.
        # B14 DIBreakoutPyramid (S=1.23) uses: DI spread < 5 OR close < Donchian low OR ADX < 18
        if self.position and self.entry_price:
            signal_exit = False
            # REPLACE these with your strategy's actual reversal conditions.
            # B14 (S=1.23) uses: DI spread < 5 OR close < Donchian low OR ADX < 18
            # signal_exit = (self.data.Close[-1] < self.sma200[-1] or
            #                <indicator crossed bearish> or
            #                <momentum < 0>)
            if signal_exit:
                self.position.close()
                print(f"Devantsa: EXIT signal at {{self.data.Close[-1]:.2f}}, held {{self.bars_held}} bars")
                self.entry_price = None
                self.trailing_stop = None
                self.highest_since_entry = None
                self.units = 0
        ...
        if self.position and self.entry_price:
            ...
            # Adaptive trail: tightens as profit grows (proven 1.5R/3.0R thresholds)
            profit_pct = (self.data.Close[-1] - self.entry_price) / self.entry_price
            initial_risk_pct = 2.5 * atr / self.entry_price
            risk_multiples = profit_pct / max(initial_risk_pct, 0.001)

            if risk_multiples >= 3.0:
                trail_mult = 2.0   # Tight trail at 3R+ profit
            elif risk_multiples >= 1.5:
                trail_mult = 2.5   # Medium trail at 1.5R+ profit
            else:
            ...
            if self.data.Low[-1] <= self.trailing_stop:
                ...
                self.units = 0
                self.bars_held = 0
            # Max hold safety valve (150 bars for 4h pyramid strategies)
...
LEGACY MODE 1 - SIMPLE TRAILING STOP (only if idea says "no pyramid"):
```python
...
# - Fixed trail multiplier (3.0x ATR, no adaptive tightening)
# - Max hold 150 bars
```

LEGACY MODE 2 - FIXED TP (only if idea explicitly says "fixed TP" or "scalp"):
```python
entry_price = self.data.Close[-1]
atr = self.atr[-1]
stop_price = entry_price - 4.0 * atr
...
KEY SIZING RULES:
- Default risk: 0.5% per unit (raw_size = 0.005 / ...) -- proven optimal for SOL (B16 sweep: S=1.48)
  Range 0.4-0.7% is acceptable. 0.5% gives best SOL Sharpe, 0.7% for BTC strategies.
- Max 2 units = 1.0% total exposure (MAX_UNITS=2 proven > 3 for Sharpe)
- Always clamp: max(0.01, min(0.5, raw_size))
- Always use ATR-based stops (no fixed % stops)
- Fresh ATR for each pyramid unit's sizing
- Adaptive trail: 3.0x ATR at 0R, 2.5x at 1.5R, 2.0x at 3.0R profit
- Initial stop: 2.5x ATR (proven Batch 11)
- Max hold: 150 bars for 4h data

========================================================================
 PROVEN WINNING PATTERNS (1h era - STRUCTURAL REFERENCE ONLY)
========================================================================
NOTE: These were 1h-calibrated. Use the BATCH 11/12 WINNERS above for 4h parameters.

BitemporalThrust (Sharpe 1.14-1.26 on SOL, best overall risk-adjusted):
- 3 conditions: momentum_fast(8) > 0.05, momentum_slow(35) > 0.03, close > SMA(100)
- Volume > 1.6x avg, trailing stop 3.0x ATR
- Dual timeframe momentum confirmation is the strongest signal pattern

HullReentry (+19.15% on BTC, +30.14% on SOL):
- 3 conditions: Hull MA slope positive 5 bars, price pulls back to Hull, closes above
- Volume > 1.5x avg, trailing stop with initial 3.5x ATR

VolatilitySurge (+0.70% PROFITABLE in bear, 28 trades):
- 3 conditions: close > 25-bar high, momentum(10) > 6%, volume > 1.8x avg
- Self-gates out of bear markets with strict conditions

Pattern: Trend confirmation + Momentum + Volume + TRAILING STOP = ride the full move.

========================================================================
 PROVEN WINNER PATTERNS (CURRENT PORTFOLIO v8)
========================================================================

B14 DIBreakoutPyramid (BTC-4h, S=1.23, +82.1%, DD=-6.66%, 156 trades) - BEST EVER:
- DI spread > 18 + Donchian(20) + SMA(200). NO volume filter.
- Pyramid ADD: DI spread > 20 (widening confirmation)
- SIGNAL EXITS: DI spread < 5 OR close < Donchian low OR ADX < 18
- MAX_UNITS=2, 0.7% risk per unit, 2.5x ATR stop
- Sweep-optimized (57 BT): risk 1.0%->0.7%, DI entry 12->18
- KEY: DI spread 18 > 12 (selectivity INCREASES return AND Sharpe)

T09 DirectionalIgnition (BTC-4h, S=1.23, +17.0%, DD=-2.73%, 36 trades) - TIGHTEST DD:
- ADX(14) crosses above 25 + EMA(21) stack + SMA(200)
- Sweep-optimized (294 BT): adx_thresh 22->25, ema_span 25->21
- Walk-forward validated (WF 6/9, WFO 2/3)

T07 ATRExpansionBreakout (BTC-4h, S=1.22, +35.2%, DD=-3.55%, 96 trades) - MOST DIVERSIFIED:
- ATR(9) > 1.5x ATR(40) + Donchian(20) + SMA(200)
- Sweep-optimized (342 BT): atr_slow 34->40, vol_mult 1.2->1.5
- Walk-forward validated (WF 7/9, WFO 2/3)

B15 DualDonchianBreakout (BTC-4h, S=1.14, +60.0%, DD=-7.67%, 205 trades):
- Close above BOTH 12-bar AND 20-bar highest highs + EMA(14) > SMA(200)
- Sweep-optimized (93 BT): stop 3.0x + EMA 14

B15 RelativeBreakout (BTC-4h, S=1.00, +48.7%, DD=-7.08%, 252 trades):
- Close/SMA(50) at new 10-bar high + SMA(200)
- Sweep-optimized (90 BT): risk 0.7% + RS exit threshold 8
- NEW signal family: Close/SMA ratio breakout

T10 DualROCAlignment (BTC-1h, S=1.21, +61.6%, DD=-6.68%, 72 trades) - ONLY 1H STRATEGY:
- ROC(8) > 2% + ROC(17) > 0% + ROC(8) > ROC(17) + SMA(240) + EMA(14)>EMA(31) + Volume > 3.0x
- Sweep-optimized (288 BT): roc_fast 6->8, stop 2.0->1.5
- Dual-timescale ROC alignment = catching ACCELERATION

KEY LESSONS FROM PORTFOLIO v8 (1227 total sweep backtests):
1. SIGNAL EXITS are THE Sharpe driver -- every S>1.0 strategy has them. MANDATORY.
2. 0.5% risk per unit = optimal Sharpe for SOL (B16 sweep proven: S=1.48)
3. Sweep optimization delivers 0.1-0.3 Sharpe improvement per strategy
4. Selectivity INCREASES return AND Sharpe (DI 18>12, volume 2.0x>1.3x)
5. Volume filter MANDATORY for SOL/ETH. 1.5-2.0x rolling average.
6. MAX_UNITS=2 > 3 for Sharpe. 2.5x ATR stop > 3.0x.
7. Shorter periods (3-10 bars) work better on SOL (fast mover). slope_period=3 beat 5.
8. Multi-exit systems resist sweep optimization (5-7 of 11 axes already optimal)
9. Trail thresholds 1.5R/3.0R > 2R/4R (faster tightening)
10. Stop 3.0x HELPS Donchian strategies (signal exits handle exits, wider stop prevents premature ATR-trail exits)

PROVEN SOL WINNER (B16 SteepeningSlopeBreakout, SOL-4h, S=1.48, +34.8%, DD=-3.3%, 170 trades):
- EMA(21) slope (3-bar diff) > 2x its 20-bar avg + close > EMA(21) + SMA200 + volume > 2.0x avg
- Pyramid ADD: slope > 1.5x avg AND profitable
- SIGNAL EXITS: slope < avg OR close < EMA(21)
- 0.5% risk, 2.5x ATR stop. KEY: slope_period=3 (fast), volume 2.0x (ultra-selective)
...
Mean reversion (RSI < 30, Williams < -70) does NOT work in crypto. Use momentum/breakout.

========================================================================
 VOLUME RULES
========================================================================
Volume filter behavior depends on asset:

BTC: Volume filter is OPTIONAL. BTC volume spikes often correlate with reversals, not continuations.
  - If used on BTC: Volume > 1.0-1.2x rolling average (low bar) or omit entirely
  - DualROCAlignment (BTC winner) was optimized to 3.0x -- ultra-selective works BUT
    generic 1.5x volume kills BTC entries without adding edge

SOL/ETH: Volume filter is RECOMMENDED (boolean, not percentile):
  - Volume > 1.5-2.0x rolling 20-bar average
  - OBV[-1] > OBV[-2] (directional)

ALL ASSETS:
1. Volume as BOOLEAN only (never percentile rank on volume/OBV/MFI/AD)
2. SINGLE volume filter per entry (not multiple volume checks)
3. Do NOT rank volume -- volume is lognormal with extreme outliers

# DEVANTSA_VOLUME_V4_BTC_OPTIONAL

========================================================================
 SQUEEZE/COMPRESSION STRATEGIES (TWO-STAGE LOGIC)
========================================================================
NEVER check compression AND expansion on the SAME bar - they are opposite states!
Low volatility (compression) and big movement (expansion) cannot coexist on one bar.

CORRECT (time-separated):
```python
compressed_recently = bb_width_pct[-2] < 0.35 or bb_width_pct[-3] < 0.35
expanding_now = roc_pct[-1] > 0.70
volume_surge = self.data.Volume[-1] > 1.8 * vol_avg[-1]

if compressed_recently and expanding_now and volume_surge and not self.position:
    self.buy(size=size)
```

Alternative - detect squeeze EXIT (width expanding):
```python
was_squeezed = bb_width_pct[-2] < 0.25
now_expanding = bb_width_pct[-1] > 0.40
momentum_strong = roc_pct[-1] > 0.65

if was_squeezed and now_expanding and momentum_strong:
    self.buy(size=size)
```
...
If strategy name contains "Squeeze", "Compression", "Contraction", or "Coil":
YOU MUST use two-stage logic with time separation!

========================================================================
 PULLBACK DETECTION RULES
========================================================================
Pullback condition MUST be a SINGLE simple comparison:
- close < sma20 (price pullback)
- rsi < 60 (momentum pullback)
- close < sma50 * 1.03 (percentage pullback)

NO compound "AND" operators within the pullback condition itself.
Maximum 4 total entry filters: trend + pullback + turning + volume.

MANDATORY PULLBACK PATTERN #1 (Simple Price Pullback):
```python
trend_ok = self.data.Close[-1] > sma200[-1] and adx[-1] > 25
pullback_ok = self.data.Close[-1] < sma20[-1]       # SIMPLE!
turning_ok = self.data.Close[-1] > self.data.Close[-2]  # Price upturn
volume_ok = self.data.Volume[-1] > 1.5 * vol_avg[-1]

if trend_ok and pullback_ok and turning_ok and volume_ok and not self.position:
    self.buy(size=size)
```

MANDATORY PULLBACK PATTERN #2 (RSI Pullback):
```python
trend_ok = self.data.Close[-1] > sma200[-1]
pullback_ok = rsi[-1] < 60  # Not oversold, just pulled back
turning_ok = rsi[-1] > rsi[-2]  # RSI turning up
volume_ok = self.data.Volume[-1] > 1.5 * vol_avg[-1]
...
If strategy name contains "Pullback", "Retracement", "Dip", or "Bounce":
YOU MUST use one of the mandatory patterns above!
If your pullback strategy generates <20 trades, your pattern is too strict.
...
- Mean reversion in crypto (RSI < 30, Williams < -70 generates 0 trades)
...
- Bare self.buy() without size parameter (full equity = catastrophic risk)
...
- Compound pullback conditions with AND (use single comparison)
- Multiple volume conditions in entry (use exactly one)
- Entry without at least one trend filter (SMA200, EMA stack, or ADX>25)
- Pyramid add without profitability check (must check self.position.pl > 0 for units 2+)
- Fixed trail multiplier on pyramid strategies (must use adaptive: 3.0x/2.5x/2.0x at 0R/1.5R/3.0R)
- MAX_UNITS > 2 (standard is 2, proven Batch 11)
- MAX_UNITS < 2 (minimum 2 units for pyramid strategies)
- Trade frequency > 300 on 4h data = strategy is churning, add stricter filters
...
3. Entry/exit logic with 3+ conditions (trend filter + breakout/momentum + optional volume)
4. Risk management (0.5% per trade, ATR-based stops)
...
if __name__ == "__main__":
    ...
    data = pd.read_csv('{DATA_PATH}\\\\BTC-USD-4h.csv')
    ...
    if results:
        ...
        results_csv_path = f'results/YourStrategyName.csv'
...
5. Minimum 3 entry conditions (trend filter + signal + confirmation). Volume is OPTIONAL for BTC.
6. 1% risk per trade position sizing with size clamping: max(0.01, min(0.5, raw_size))
7. Use MODE 3 PYRAMID + ADAPTIVE TRAIL by default (self.units counter, adaptive trail_mult, all units close at once)
8. Only use simple trailing (MODE 1) if idea says "no pyramid", or fixed TP (MODE 2) if idea says "fixed TP"
9. Every strategy MUST include a trend filter (Close > SMA200, EMA21 > EMA50, or ADX > 25)

Use this data path: {DATA_PATH}\\BTC-USD-4h.csv
The data format:
...
4. Position Sizing:
   ...
   - ALWAYS validate: if size > 0 and size < 1: self.buy(size=size)
...
8. No Trades (0 trades output):
   - Check self.buy() is actually called in entry condition
...
YOUR MISSION: Maximize COMPOSITE SCORE (50% Sharpe + 15% Return + 25% DD protection + 10% Win Rate).
Sharpe improvement of 0.3 is as valuable as 10% return improvement.

BTC-4H FOCUS: This agent hunts BTC-4h alpha. If BTC-4h Sharpe is below 0.5 in multi-data results,
focus optimization EXCLUSIVELY on improving BTC-4h performance. Do NOT sacrifice BTC-4h Sharpe
for SOL/ETH gains. SOL performance is irrelevant if BTC doesn't qualify.

PROP FIRM CONSTRAINT (Crypto Fund Trader): Max drawdown MUST stay under 5%.
Strategies with DD worse than -5% are UNUSABLE for prop firm challenges.
Prioritize DD protection over return when DD approaches -5%.

CRITICAL SHARPE FLOOR RULE: If the current Sharpe is above 1.2, do NOT accept changes that drop
it below 1.0 even if composite score improves. Protect high Sharpe at all costs.
Sharpe > 1.5 is the PRIMARY TARGET. Higher returns are OK only if Sharpe holds.

TRADE COUNT WARNING: If trade count > 250 on 4h data, consider adding stricter entry filters.
If trade count > 300, the strategy is CHURNING. Add tighter momentum gate or stacked EMA filter.
NOTE: B14 DIBreakoutPyramid has 156 trades at S=1.23 and B15 RelativeBreakout has 252 at S=1.00.
High trade count is FINE if Sharpe holds above 1.0.

ITERATION-SPECIFIC FOCUS (you MUST try something DIFFERENT each iteration):
- Iteration 1: Reduce risk to 0.5% per unit (proven optimal for SOL, B16 sweep: S=1.48)
- Iteration 2: Increase volume selectivity to 2.0x average (ultra-selective = #1 DD fixer on SOL)
- Iteration 3: Add or improve SIGNAL EXITS (slope reversal, indicator cross-back, close below key MA)
- Iteration 4: Shorten indicator periods (3-8 bars for SOL -- fast mover). slope_period=3 beat 5 on SOL.
- Iteration 5: Increase entry selectivity (tighter thresholds, additional conditions)
- Iteration 6: Combine best parameter changes from iterations 1-5. Target: Sharpe > 1.0
...
- DO NOT remove pyramid logic! Keep self.units counter, MAX_UNITS, and profitability check for units 2+.
- DO NOT remove adaptive trailing stop! Keep the R-multiple based trail tightening (3.0x/2.5x/2.0x at 0R/1.5R/3.0R).
- DO NOT remove the trend filter (SMA200, EMA stack, or ADX). You may swap one filter for another.
- DO NOT change MAX_UNITS above 3 or below 1. Standard is 2.
- DO NOT remove the profitability check for pyramid adds (self.position.pl > 0).
- DO NOT reduce position sizing below 0.3% risk per unit. Optimal is 0.5% for SOL (B16 sweep proven).
...
def calculate_composite_score(stats: dict, thread_id: int) -> float:
    ...
    - 50% Sharpe Ratio (risk-adjusted returns)
    - 15% Raw Returns (scaled to 0-1 range)
    - 25% Drawdown Protection (inverse of max drawdown)
...
def parse_and_log_multi_data_results(strategy_name: str, thread_id: int, backtest_file_path: Path) -> None:
    ...
    try:
        ...
        results_dir = RBI_REGULAR_DIR / "results"
...
def main(ideas_file_path=None, run_name=None, fulldata=True):
    ...
    if ideas_file_path:
        ...
        # Use the main ideas.txt in RBI_Regular/
        IDEAS_FILE = RBI_REGULAR_DIR / "ideas.txt"
