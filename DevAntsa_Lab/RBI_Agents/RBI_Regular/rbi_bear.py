...
TARGET_RETURN = 75  # Batch 8: raised from 50. Sweep-optimized winners hit S=1.16-1.46. Push harder.
...
MAX_OPTIMIZATION_ITERATIONS = 6  # Reduced from 10: v3-v5 is sweet spot, v6+ over-fits and cuts risk
...
PROCESSED_IDEAS_LOG = DATA_DIR / "processed_ideas_bear.log"
STATS_CSV = DATA_DIR / "backtest_stats_bear.csv"
IDEAS_FILE = RBI_REGULAR_DIR / "ideas_bear.txt"
CURRENT_REGIME = None
...
def update_date_folders():
    ...
    with date_lock:
        ...
        WINNERS_DIR = RBI_REGULAR_DIR / "winners_bear"
...
2. CRITICAL: If the idea starts with "Thread T##:", your strategy name MUST start with that prefix.
   Example: If idea says "Thread T05:", your name must be "T05_AdaptiveBreakout"
   Example: If idea says "Thread T12:", your name must be "T12_FractalMomentum"
3. The name must be DIFFERENT from any generic names like "TrendFollower" or "MomentumStrategy"
4. First word (after prefix) should describe the main approach (e.g., Adaptive, Neural, Quantum, Fractal, Dynamic)
5. Second word should describe the specific technique (e.g., Reversal, Breakout, Oscillator, Divergence)
6. Make the name SPECIFIC to this strategy's unique aspects

Examples of good names:
- "T05_AdaptiveBreakout" for thread 5's strategy that adjusts breakout levels
- "T12_FractalMomentum" for thread 12's strategy using fractal analysis with momentum
- "T01_QuantumReversal" for thread 1's complex mean reversion strategy

BAD names to avoid:
- "TrendFollower" (too generic)
- "SimpleMoving" (too basic)
- "PriceAction" (too vague)
- Names without thread prefix when one was provided

Output format must start with:
STRATEGY_NAME: [Your unique two-word name with thread prefix]
...
BACKTEST_PROMPT = """
You are Devantsa's Backtest AI generating SHORT-ONLY bear market strategies.

========================================================================
 TOP RULES (READ FIRST - THESE OVERRIDE EVERYTHING BELOW)
========================================================================
1. SHORT ONLY: Use self.sell(size=size) for entries. NEVER self.buy().
2. BEAR GATE: Every entry MUST check price < SMA(200). Non-negotiable.
3. TRAILING STOP REVERSED: Track lowest_since_entry. Trail ABOVE price.
4. 4H DATA: Strategies run on 4h candles. Use periods 10-30 for indicators.
5. NO EMOJIS: Windows CP1252 crashes. Plain ASCII only in print().
6. MINIMUM 3 CONDITIONS: Bear gate + signal + volume (or momentum).

========================================================================
 WINDOWS COMPATIBILITY - NO EMOJIS!
========================================================================
NO EMOJIS OR UNICODE in print() statements! Windows CP1252 encoding crashes.
Use ONLY plain ASCII: print("Devantsa: SHORT signal detected")
CHECK EVERY print() statement - NO EXCEPTIONS!

========================================================================
 BACKTESTING.PY API RULES
========================================================================

RULE 1: _Array objects are NOT pandas Series!
backtesting.py uses _Array objects that DO NOT support:
.shift(), .rolling(), .iloc[], .values, .pct_change()

WRONG (crashes):
```python
def init(self):
    close_series = self.data.Close.shift(1)  # CRASH
    rolling_mean = self.data.Close.rolling(20).mean()  # CRASH
```

CORRECT (use self.I() wrapper):
```python
def init(self):
    self.close_shifted = self.I(lambda x: pd.Series(x).shift(1), self.data.Close)
    self.rolling_mean = self.I(lambda x: pd.Series(x).rolling(20).mean(), self.data.Close)
```

ALL indicator calculations MUST be wrapped in self.I().
...
# Usage: self.rsi = self.I(RSI, self.data.Close, 14)
```

Other indicator patterns:
- SMA: self.I(lambda x, n: pd.Series(x).rolling(n).mean(), self.data.Close, 20)
- EMA: self.I(lambda x, n: pd.Series(x).ewm(span=n).mean(), self.data.Close, 20)

RULE 2: Position object has NO .entry_price, .entry_bar, .sl, .tp
...
class MyStrategy(Strategy):
    ...
    def next(self):
        ...
        if not self.position and <entry_condition>:
            ...
            self.sell(size=size)  # SHORT entry

        if self.position and self.entry_price:
            # SHORT profit: entry_price - current_price
            if self.data.Close[-1] < self.entry_price * 0.95:  # 5% drop = profit for short
                self.position.close()
                self.entry_price = None
```
Position DOES have: .size, .pl, .pl_pct, .is_long, .is_short

RULE 3: Position sizing - MUST validate and clamp!
```python
raw_size = 0.01 / (stop_dist / entry_price)  # 1% risk per trade
size = max(0.01, min(0.5, raw_size))  # Clamp 1%-50%
if size > 0 and size < 1:
    self.sell(size=size)  # SHORT entry
```

RULE 4: No talib! Use pandas/numpy equivalents (RSI, SMA, EMA examples above).

RULE 5: No backtesting.lib.crossover()! Use array indexing:
  Bullish crossover: if signal[-2] < reference[-2] and signal[-1] > reference[-1]
  Bearish crossover: if signal[-2] > reference[-2] and signal[-1] < reference[-1]

Example - SMA death cross SHORT strategy:
```python
fast_ma = self.I(lambda x: pd.Series(x).rolling(20).mean(), self.data.Close)
slow_ma = self.I(lambda x: pd.Series(x).rolling(50).mean(), self.data.Close)

# SHORT on bearish crossover (death cross)
if fast_ma[-2] > slow_ma[-2] and fast_ma[-1] < slow_ma[-1]:
    if not self.position:
        self.sell(size=size)  # SHORT entry

# Cover on bullish crossover
if fast_ma[-2] < slow_ma[-2] and fast_ma[-1] > slow_ma[-1]:
    if self.position:
        self.position.close()  # Close short
```

RULE 6: In next(), _Array uses NEGATIVE indexing only:
  [-1] = current bar, [-2] = previous bar, [-3] = two bars ago
  NO .shift(), .rolling(), .iloc(), .loc(), .values on _Array objects
  Use self.equity for account balance (NOT self._broker.*)

RULE 7: No bt.plot()! Causes timeouts in parallel processing. Print stats only.

RULE 8: Only print inside next() on TRADE ENTRY/EXIT, NOT on every bar!

========================================================================
END OF BACKTESTING.PY API RULES
========================================================================

========================================================================
 FULL-DATA SELF-GATING PRINCIPLE (SHORT STRATEGIES)
========================================================================
Strategies are tested on ALL market data (bull + crash + bear + recovery + sideways, ~5 years).
Strict SHORT entry conditions simply don't trigger in bull markets - the entry conditions ARE
the regime filter. No regime detector needed.

KEY EVIDENCE: Bear strategies with price < SMA200 gate had only 2-5 trades during full bull
periods (+1.14% to -3.70%). They self-gate out of bulls by requiring bearish structure.
The 3 proven ETH-4h bear strategies all have 3+ strict conditions and survive 5yr full data.

SHORT strategies MUST require price BELOW SMA200 as primary gate. This is non-negotiable.

========================================================================
 MANDATORY ENTRY CONDITION RULES (SHORT STRATEGIES)
========================================================================
MINIMUM 3 independent entry conditions. Strategies with 1-2 conditions churn in bull markets.
Adding a 4th condition is RECOMMENDED for higher Sharpe and better self-gating.

PRIMARY BEAR GATE (MANDATORY - non-negotiable):
- Price < SMA(200): macro bear gate. This MUST be condition #1.

SECONDARY BEAR CONFIRMATION (at least one required):
- Declining ROC: ROC(14) or ROC(21) negative (momentum confirming downtrend)
- EMA death cross: EMA(short) < EMA(medium) < EMA(long) (inverted stacked EMA)
- ADX > 25 with -DI > +DI: trend strength confirming bear
- Lower highs: current 20-bar high < previous 20-bar high

Volume confirmation (boolean, not percentile) counts as one condition:
- Volume > 1.5-2.0x rolling 20-bar average

RSI entry guidance (two proven approaches):
- RSI(14) < 45: PROVEN cliff (sweep: EMARejectionADX +0.21 Sharpe at 45 vs 48). Use for momentum/structural entries.
- RSI(14) 45-65: NOT-oversold filter (GradualBleed SOL S=1.03). Use for slow-bleed entries.
- RSI(14) 60-70: rally exhaustion fade (use ONLY for explicit fade strategies).

CORRECT (3+ conditions, self-gating SHORT):
```python
bear_ok = self.data.Close[-1] < self.sma200[-1]               # Condition 1: BEAR gate
momentum_down = self.roc[-1] < 0                               # Condition 2: declining momentum
volume = self.data.Volume[-1] > 1.8 * self.vol_avg[-1]        # Condition 3: volume
# BONUS (recommended for higher Sharpe):
# ema_death = self.ema12[-1] < self.ema26[-1] < self.ema50[-1]  # Condition 4: death cross

if bear_ok and momentum_down and volume and not self.position:
    self.sell(size=size)  # SHORT entry - use self.sell(), NOT self.buy()!
```

WRONG (no bear gate, will fire in bull markets):
```python
if self.rsi[-1] > 70:  # Single condition = fires constantly in bulls
    self.sell()  # Will short into rallies and get destroyed
```

========================================================================
 PRE-BAKED BEAR GATE (MANDATORY)
========================================================================
EVERY SHORT strategy MUST include price < SMA(200) as an entry condition.
Additional bear confirmation filters improve Sharpe and self-gating.

CORRECT (bear gate + death cross bonus):
```python
bear_ok = self.data.Close[-1] < self.sma200[-1]              # MANDATORY: bear gate
ema_death = self.ema12[-1] < self.ema26[-1] < self.ema50[-1]  # BONUS: death cross
breakdown = self.data.Close[-1] < self.low_25[-1]             # Condition 3: breakdown
volume_ok = self.data.Volume[-1] > 1.7 * vol_avg[-1]         # Condition 4: volume

if bear_ok and ema_death and breakdown and volume_ok:
    self.sell(size=size)  # SHORT entry
```

WRONG (no bear gate - will short into bull rallies):
```python
if self.rsi[-1] > 70 and self.momentum[-1] < 0:
    self.sell(size=size)  # Will short in bull markets and get destroyed
```

========================================================================
 TRADE FREQUENCY TARGETS (4H DATA, 5 YEARS)
========================================================================
System uses 4h data (BTC/ETH/SOL 4h, ~5 years from Jan 2021).
4h timeframe is MANDATORY for bear shorts - every 1h short tested negative.
With trailing stops, each trade is held longer = fewer total trades needed.

- 4h data: 30-80 trades (ideal: 35-60 for highest Sharpe)
- Strategies with 100+ trades are CHURNING - auto-rejected
- Strategies with <10 trades are statistically meaningless

Commission drag (0.1% per side, 0.2% round-trip - 1.67x buffer over Bybit taker 0.06%):
- 30 trades = 6% drag
- 60 trades = 12% drag (sweet spot with trailing)
- 80 trades = 16% drag (upper limit for high-expectancy)

PROP FIRM CONTEXT (Crypto Fund Trader):
- Max total DD: 5-6% depending on challenge type
- Target profit: 8-10% to pass challenge
- Strategies must be CONSERVATIVE on drawdown
Use indicator periods (10-30 bars) for appropriate signal frequency on 4h.

========================================================================
 RISK MANAGEMENT
========================================================================
Three modes available. Use MODE 3 (default) unless idea explicitly says otherwise.

MODE 3 - SHORT PYRAMID + ADAPTIVE TRAIL (DEFAULT for all bear strategies):
Stacks up to 2 SHORT units on winning positions. Trail tightens as profit grows.
Each unit risks 1.0-1.2% (2.0-2.4% total max exposure). All units close at once on trail hit.
4H NOTE: ATR(14) = ~2.3 days. SMA(200) = ~33 days. Use periods 10-30 for 4h signals.

CRITICAL SHORT DIFFERENCES FROM LONG:
- Entry: self.sell(size=size) NOT self.buy(size=size)
- Track LOWEST since entry (not highest)
- Trail stop is ABOVE price (not below)
- Profit = (entry_price - current_price) / entry_price (reversed)
- Exit when price RISES above trail (not falls below)

```python
class MyStrategy(Strategy):
    MAX_UNITS = 2  # Batch 7: reduced from 3. Proven higher Sharpe in bull Batch 11.
    def init(self):
        ...
        ).rolling(17).mean(), self.data.High, self.data.Low, self.data.Close)
        self.entry_price = None
        self.trailing_stop = None
        self.bars_held = 0
        self.units = 0
        self.lowest_since_entry = None  # Track LOWEST for shorts
    ...
    def next(self):
        ...
        # REVERSAL EXIT: Close ALL units if bear thesis breaks (BEFORE entry logic)
        if self.position and self.entry_price:
            if self.data.Close[-1] >= self.sma200[-1]:
                self.position.close()
                print(f"Devantsa: EXIT reversal ({{self.units}} units) at {{self.data.Close[-1]:.2f}} - price reclaimed SMA200")
                self.entry_price = None
                self.trailing_stop = None
        ...
        # PYRAMID SHORT ENTRY: same conditions for all units
        can_enter = self.units < self.MAX_UNITS
        if can_enter and self.units > 0:
            # Pyramid only if existing SHORT position is profitable
            if not self.position or self.position.pl <= 0:
                can_enter = False

        if can_enter:
            bear_ok = self.data.Close[-1] < self.sma200[-1]  # BEAR gate
            entry_signal = bear_ok and <cond2> and <cond3> and <volume>
            if entry_signal:
                entry_price = self.data.Close[-1]
                stop_dist = 1.8 * atr  # Batch 8: sweep-optimized from 2.0x. EP S 0.74->1.10 with 1.8x.
                raw_size = 0.012 / (stop_dist / entry_price)  # 1.2% risk per unit (sweep: EP optimal at 1.2%)
                size = max(0.01, min(0.5, raw_size))
                if size > 0 and size < 1:
                    self.sell(size=size)  # SHORT entry!
                    self.units += 1
                    print(f"Devantsa: SHORT unit {{self.units}} at {{entry_price:.2f}}")
                    if self.units == 1:
                        self.entry_price = entry_price
                        self.trailing_stop = entry_price + stop_dist  # Trail ABOVE for shorts
                        self.lowest_since_entry = self.data.Low[-1]  # Track LOWEST
                        self.bars_held = 0

        # EXIT - adaptive trailing stop management (REVERSED FOR SHORTS)
        if self.position and self.entry_price:
            self.bars_held += 1
            # Track LOWEST price since entry (not highest)
            if self.data.Low[-1] < self.lowest_since_entry:
                self.lowest_since_entry = self.data.Low[-1]

            # Adaptive trail: tightens as profit grows (REVERSED for shorts)
            profit_pct = (self.entry_price - self.data.Close[-1]) / self.entry_price
            initial_risk_pct = 1.8 * atr / self.entry_price  # Must match stop_dist (1.8x)
            risk_multiples = profit_pct / max(initial_risk_pct, 0.001)

            # Batch 9: sweep-optimized R-thresholds (0.8R/2.5R). SB sweep: 1.5x mid-trail > 1.8x.
            if risk_multiples >= 2.5:
                trail_mult = 1.5   # Tight trail at 2.5R+ profit
            elif risk_multiples >= 0.8:
                trail_mult = 1.5   # Medium trail at 0.8R+ (was 1.8, SB sweep: ETH S 1.058->1.328)
            else:
                trail_mult = 2.0   # Initial trail

            # Trail ABOVE price for shorts
            new_trail = self.lowest_since_entry + trail_mult * atr
            if new_trail < self.trailing_stop:  # LOWER is better for short trail
                self.trailing_stop = new_trail

            # BREAK-EVEN AT 1R: Once profitable by 1R, cap stop at entry price (zero risk)
            if risk_multiples >= 1.0:
                self.trailing_stop = min(self.trailing_stop, self.entry_price)

            # Exit ALL units when price RISES above trail
            if self.data.High[-1] >= self.trailing_stop:
                ...
                self.units = 0
                self.bars_held = 0
            # Max hold safety valve (150 bars -- sweep: tighter = better Sharpe)
            elif self.bars_held >= 150:
                self.position.close()
...
LEGACY MODE 1 - SIMPLE SHORT TRAILING STOP (only if idea says "no pyramid"):
```python
# Same as MODE 3 but without pyramiding:
# - self.units not needed, use "if not self.position:" for entry
# - No profitability check for adds
# - Fixed trail multiplier (3.0x ATR above lowest, no adaptive tightening)
# - self.sell(size=size) for entry, track lowest_since_entry
# - Max hold 150 bars
```

LEGACY MODE 2 - FIXED TP SHORT (only if idea explicitly says "fixed TP" or "scalp"):
```python
entry_price = self.data.Close[-1]
atr = self.atr[-1]
stop_price = entry_price + 2.5 * atr   # Stop ABOVE for shorts (tighter = more profit)
tp_price = entry_price - 5.0 * atr     # TP BELOW for shorts
stop_dist = abs(stop_price - entry_price)
raw_size = 0.01 / (stop_dist / entry_price)   # 1% risk
size = max(0.01, min(0.5, raw_size))

if size > 0 and size < 1:
    self.sell(size=size, sl=stop_price, tp=tp_price)  # SHORT entry
```

KEY SIZING RULES (SHORT STRATEGIES):
- MANDATORY risk: 1.0-1.2% per unit (raw_size = 0.012 / ...). Do NOT use 0.5% - returns are too low.
- Optimizer may increase to 1.5% per unit if DD is low. 1.5% is the MAX for prop firm safety.
- Max 2 units = 2.0-2.4% total exposure (MAX_UNITS=2, proven higher Sharpe than 3)
- Always clamp: max(0.01, min(0.5, raw_size))
- Always use ATR-based stops (no fixed % stops)
- Fresh ATR for each pyramid unit's sizing
- SHORT trail: track lowest_since_entry + trail_mult * self.atr[-1] (DYNAMIC current ATR, NOT fixed)
- Adaptive trail: 2.0x ATR at 0R, 1.5x at 0.8R, 1.5x at 2.5R profit (sweep-optimized Feb 2026)
- BREAK-EVEN AT 1R: Once profit >= 1R, cap trailing stop at entry_price (zero risk on winners)
- Initial stop: entry_price + 1.8x ATR for structural/fade entries, 1.2x ATR for momentum/acceleration
- Max hold: 150 bars for pyramid/trailing strategies, 50 for fixed TP
- Use self.sell(size=size) NOT self.buy(size=size)!

========================================================================
 REVERSAL EXIT (MANDATORY FOR ALL BEAR STRATEGIES)
========================================================================
Add a REVERSAL EXIT that closes ALL units immediately when the bear thesis breaks.
This MUST appear at the START of next() BEFORE entry logic.

The simplest and most effective reversal exit: close if price reclaims SMA200.
Optional additions: close if momentum turns positive, or ATR contracts below average.

```python
# REVERSAL EXIT (place BEFORE entry logic in next())
if self.position and self.entry_price:
    if self.data.Close[-1] >= self.sma200[-1]:
        self.position.close()
        print(f"Devantsa: EXIT reversal ({{self.units}} units) at {{self.data.Close[-1]:.2f}}")
        self.entry_price = None
        self.trailing_stop = None
        self.lowest_since_entry = None
        self.units = 0
        self.bars_held = 0
        return  # Skip entry logic this bar
```

WHY: The best Batch 3 strategy (ExpansionBreakdown, +13.60%, S=0.71) uses this pattern.
Without it, strategies hold through bear-to-bull transitions and return profit.
This is the single biggest improvement over Batch 2 strategies.

========================================================================
 PROVEN WINNING BEAR PATTERNS (5-YEAR FULL DATA TESTED)
========================================================================
ConfluentMomentum ETH-4h (+11.01%, Sharpe 0.40, DD -8.99%, 110 trades):
- 3 conditions: close < SMA200, declining momentum (ROC negative), volume above avg
- Best self-gating: only 2 trades during full bull period (+1.14%)
- SIMPLEST bear strategy = most robust

PlungeRsiHybrid ETH-4h (+11.39%, Sharpe 0.44, DD -6.13%, 95 trades):
- RSI + momentum hybrid: RSI overbought fade + declining momentum + bear gate
- TIGHTEST drawdown of all bear strategies (-6.13%)
- Also works on SOL-4h (+7.45%, nearly flat on BTC -1.75%)
- RSI-based fades are the CLOSEST to working on BTC

VolumePlunge ETH-4h (+10.86%, Sharpe 0.37, DD -8.47%, 126 trades):
- Volume breakdown: high-volume red candles in established downtrend
- Profitable even during bull (+1.60%!)

ExpansionBreakdown BTC-4h (+13.60%, Sharpe 0.71, DD -2.65%, 40 trades) - BEST EVER:
- 5 conditions: close < SMA200, ATR > 1.3x ATR_SMA50, close < 20-bar low, momentum < -0.5*ATR, volume > 2.0x avg
- UNIQUE: Reversal exit (close if price > SMA200 OR momentum positive OR ATR contracting)
- 1.5% risk per unit (raised by optimizer from 1% when DD was low)
- Positive on ALL 3 assets: BTC +13.60%, ETH +3.54%, SOL +5.74%
- Shortest SMA (200) = minimal warmup data loss
- TEMPLATE for Batch 4 strategies

SequentialBreakdown BTC-4h (+9.19%, Sharpe 0.50, DD -2.86%, 79 trades):
- 4 conditions: close < SMA240, 3 consecutive red bars with decreasing closes, increasing volume
- Highest trade count among winners = most statistically significant
- Positive on ALL 3 assets

PlungeAcceleration BTC-4h (+27.7%, Sharpe 0.82, DD -6.24%, 38 trades) - HIGHEST RETURN EVER:
- Signal: close < SMA200, ROC(14) dropped from > -2% to < -4% in 5 bars, ATR > avg, volume spike
- Momentum plunge continuation pattern - ride the acceleration
- 1.5% risk per unit (optimizer pushed from 1%)
- BTC-only (ETH is -14.6% = strategy breaks on ETH). Single-asset winner.

StructuralRSIFade ALL-ASSET (+5.6% BTC, +9.6% ETH, +9.7% SOL, Sharpe 0.45-0.58, 70-94 trades):
- Signal: close < SMA200, RSI(14) crossed above 55 then fell below 50, close < EMA(21), volume > avg
- ONLY Batch 4 strategy positive on ALL 3 assets
- Higher trade count = statistically robust

===== BATCH 5 BREAKTHROUGHS (Sharpe-First + Dual Archetype + Break-Even) =====

StructuralFade ETH-4h (+14.8%, Sharpe 1.09, DD -2.37%, 43 trades) - FIRST BEAR S>1.0:
- Signal: close < SMA200, RSI(17) faded below 50 from above, close < 18-bar low, volume > 1.5x avg
- Optimized: RSI period 17, ATR period 17, vol avg lookback 24, structural low lookback 18
- Tight trail: 2.0x ATR initial, tightens to 1.8x at 1R, 1.5x at 2.5R. Break-even at 1R.
- ALL 3 POSITIVE: ETH +14.8% S=1.09, BTC +6.2% S=0.64, SOL +5.2% S=0.59
- TEMPLATE for Batch 6: RSI fade + structural break = highest Sharpe pattern

ExpansionSequential BTC-4h (+26.3%, Sharpe 0.97, DD -5.34%, 56 trades) - DUAL ARCHETYPE VALIDATED:
- DUAL ENTRY: ATR expansion breakdown OR 3 consecutive red candles with declining closes
- OR logic combines 2 proven entry paths in one strategy = more trades without diluting quality
- ALL 3 POSITIVE: BTC +26.3% S=0.97, ETH +8.8% S=0.34, SOL +9.0% S=0.34

SequentialRejection BTC-4h (+15.6%, Sharpe 0.87, DD -2.96%, 51 trades):
- Signal: close < SMA200, 3 red bars declining + EMA rejection, volume spike
- TIGHT DD (-2.96%) with decent Sharpe. ALL 3 POSITIVE.

FadingVolatilityReversal BTC-4h (+16.4%, Sharpe 0.86, DD -3.48%, 31 trades):
- Signal: close < SMA200, ATR contracting from high to low (volatility fade), RSI < 45
- BTC-specialist (SOL -5.8%). Low trade count = highest conviction entries.

Pattern: Bear gate (SMA200) + Structural breakdown/rejection + Volume + REVERSAL EXIT + BREAK-EVEN AT 1R + TRAILING STOP = ride the crash.
DUAL ARCHETYPE: Combine 2 proven entry patterns with OR logic for more trades without quality loss.

===== BATCH 7 CROSS-POLLINATION FROM BULL WINNERS =====
Bull DirectionalBreakout (S=1.20, highest ever) uses: Donchian(20) breakout + ADX(14) > 25 + DI+/DI- + NO volume filter.
Bear translation: close < Donchian(20) LOW + ADX(14) > 25 + DI-(14) > DI+(14) = confirmed directional breakdown.
ADX/DI helper functions:
```python
...
```
===== HELPER FUNCTIONS FOR VOLUME-FLOW & PRICE ACTION INDICATORS =====
If your idea uses OBV, MFI, CMF, Williams %R, or candlestick patterns, use these implementations:
...
def CMF(high, low, close, volume, period=20):
    ...
    mfv = ((2 * close - high - low) / hl_range) * volume
...
self.mfi = self.I(MFI, self.data.High, self.data.Low, self.data.Close, self.data.Volume, 14)
self.cmf = self.I(CMF, self.data.High, self.data.Low, self.data.Close, self.data.Volume, 20)
self.williams_r = self.I(WilliamsR, self.data.High, self.data.Low, self.data.Close, 14)
```

For candlestick patterns (bearish engulfing, inside bar, close-range ratio):
```python
# Bearish engulfing
engulfing = (self.data.Open[-1] > self.data.Close[-2] and
             self.data.Close[-1] < self.data.Open[-2] and
             abs(self.data.Close[-1] - self.data.Open[-1]) > abs(self.data.Close[-2] - self.data.Open[-2]))

# Inside bar (prior bar contained within bar before it)
inside_bar = (self.data.High[-2] < self.data.High[-3] and self.data.Low[-2] > self.data.Low[-3])
breakdown = self.data.Close[-1] < self.data.Low[-2]  # Break below inside bar low

# Close-range ratio (close in bottom 20% of bar = sellers dominated)
bar_range = self.data.High[-1] - self.data.Low[-1]
if bar_range > 0:
    close_range_ratio = (self.data.Close[-1] - self.data.Low[-1]) / bar_range
    close_in_bottom = close_range_ratio < 0.20
```

EMARejectionFade (S=0.85) dual reversal exit: also exits when close > EMA(50) * 1.005. Tighter than SMA200-only exit.
EscalatingPanic (S=0.74) uses 1.5% risk and SMA(230). Gets highest return (+23.87%).
KEY: MAX_UNITS=2 proven higher Sharpe than 3 (bull Batch 11). No-volume ideas valid (DirectionalBreakout has NO volume filter).

===== SWEEP OPTIMIZATION BREAKTHROUGHS (Feb 19, 2026) =====
Manual parameter sweep on top 3 bear strategies. All 3 now S>1.0.

StructuralFade ETH-4h (+18.65%, Sharpe 1.46, DD -1.24%, 33 trades, WR 69.7%) - BEST BEAR EVER:
- SWEEP: vol 1.5x->2.0x (+0.37 Sharpe), trail_r1 1.0R->0.8R (+0.07 Sharpe)
- TEMPLATE: volume selectivity > 2.0x is the single highest-leverage parameter.

EMARejectionADX ETH-4h (+13.58%, Sharpe 1.21, DD -1.64%, 61 trades, WR 55.7%):
- SWEEP: RSI 48->45 (+0.21), ADX 20->18 (+0.07), risk 1%->0.7% (tighter DD)
- TEMPLATE: multi-exit system (5 conditions) is the key differentiator. RSI<45 is a sharp cliff.

EscalatingPanic BTC-4h (+23.07%, Sharpe 1.16, DD -2.94%, 33 trades, WR 66.7%):
- SWEEP: stop 2.0x->1.8x (+0.36), DYNAMIC ATR trail (+0.42), risk 1.5%->1.2%
- CRITICAL: trail_mult * self.atr[-1] (current ATR) NOT trail_mult * (self.initial_stop - self.entry_price) (fixed)

KEY SWEEP DEFAULTS FOR BATCH 9 (proven across 540+ backtests):
- Initial stop: 1.2x ATR for acceleration/momentum entries (AB sweep: +0.204 avgS vs 1.0x)
- Initial stop: 1.8x ATR for structural/fade entries (EP sweep: +0.36 Sharpe)
- Trail: DYNAMIC ATR (trail_mult * self.atr[-1])
- Trail R-thresholds: 1.5x at 0.8R (SB sweep: 1.5x > 1.8x for mid-trail)
- ROC-adaptive trail: if ROC[-1] > ROC[-10] then 1.5x else 2.0x (AB pattern, at 1R+ only)
- Volume: > 2.0x avg (SF sweep: +0.37 Sharpe)
- RSI: < 45 (EMADX sweep: cliff at 45/46)
- ADX: > 18 (sweep: slightly better than 20)
- Risk: 1.0-1.2% per unit
- Accel lookback: 10 bars (AB sweep: 6->10 = +0.090 avgS)
- Multi-exit = grafting EMADX exit suite onto other entries

===== BATCH 8 SWEEP BREAKTHROUGHS (Feb 19, 2026) =====
AccelBreakdown (BEST DIVERSIFIED: avgS=1.011, ALL 3 assets S>0.7):
- Entry: SMA200 + ROC(10)<-2 + ROC[-1]<ROC[-10] (acceleration) + close<18-bar low + vol>2.0x
- SWEEP: stop 1.0->1.2 (+0.204 avgS = biggest single lever), accel_lb 6->10 (+0.090)
- Stop 1.2x = wider stop survives stop-hunts on volatile breakdown entries. 0.8x = worst.
- Trail: ROC-adaptive (if ROC recovering -> 1.5x, else 2.0x). Only at 1R+.
- BTC +37.4% S=0.957, ETH +69.5% S=1.354, SOL +30.1% S=0.722

StructuralBreakdown (ETH SPECIALIST: ETH S=1.328):
- Entry: SMA200 + lower high (high<max(high[-11:-5])) + close<18-bar low + RSI(17)<45 + vol>2.0x
- SWEEP: trail_m1 1.8->1.5 (ETH S 1.058->1.328 = +25%)
- Lower high = market structure confirmation. Novel entry pattern.
- BTC +7.5% S=0.368, ETH +37.9% S=1.328, SOL +8.5% S=0.381

===== BATCH 9 SWEEP 3 (Feb 19, 2026) =====
BearishLowerHigh ETH-4h (+56.7%, Sharpe 1.44, DD -3.40%) - 2ND BEST BEAR:
- SWEEP: adx 18->20, stop 1.8->1.2 = ETH S 1.343->1.444
PanicAcceleration BTC-4h (+26.6%, Sharpe 1.19, DD -2.60%):
- SWEEP: stop 1.8->2.0, RSI 42->45 = BTC S 0.927->1.188
- Wider stop + looser RSI = BTC gold (lets winners run, more entries)

===== BATCH 10 (Feb 20, 2026) - SOL/BTC DIVERSIFICATION =====
GradualBleed SOL-4h (+25.25%, Sharpe 1.03, DD -3.75%, 55 trades) - NOVEL SOL QUALIFIER:
- Signal: ROC(24) in -4.5 to -1.0 range (moderate decline, NOT panic) + ROC accelerating
- EMA(60) stacked below SMA200 + ADX(17)>22 + vol>2.0x + RSI 45-65 (NOT oversold!)
- Novel insight: RSI 45-65 = opposite of portfolio norm (RSI<45). Targets slow bleeds.
- 7 conditions produced only 55 trades = too selective. B11 targets MAX 4 conditions.
KEY B10 FINDINGS:
- Agent re-generated 3 existing strategies (EscalatingPanic, PanicAcceleration, DualBreakdown clones)
- OBV at N-bar low worked on SOL (+22.6%) but DD -12.82% = needs tighter risk
- MFI<30 worked on SOL (+11.8% S=0.67)
- HiddenDivergence = CATASTROPHIC -100% SOL (divergence BANNED)
- 7+ conditions = <20 trades = signal starvation. Use 4 MAX.

========================================================================
 ASSET-SPECIFIC PARAMETER GUIDANCE
========================================================================
The initial backtest data file below determines your PRIMARY optimization asset.
Tune parameters for THAT asset. The multi-data test runs on all 3 assets afterward.

WHEN INITIAL DATA IS SOL-USD-4h:
- SOL is MORE VOLATILE than BTC/ETH. Use wider trails (2.0-2.5x ATR), wider stops (2.0x ATR).
- SOL responds to: momentum acceleration, OBV breakdown, volume capitulation, ROC extremes
- Proven SOL winners: GradualBleed (+25.25%, S=1.03, 55 trades), ExpandingBodyBear (+18.4%, S=1.065)
- GradualBleed uses ROC(24) in -4.5 to -1.0 range + ADX>22 + vol>2.0x + RSI 45-65 (NOT oversold)
- SOL needs WIDER RSI window (45-65 vs ETH's <45). SOL oversold is different from ETH oversold.
- SOL ATR periods: 14-17. Volume threshold: 2.0x minimum.

WHEN INITIAL DATA IS BTC-USD-4h:
- BTC is SLOWER than ETH/SOL. Use patience-based entries, wider stops (2.0x ATR), looser RSI (45-50).
- BTC responds to: EMA rejection, sequential red bars, Donchian breakdowns, death crosses
- Proven BTC winners: PanicAcceleration (+26.6%, S=1.19, 33 trades), ExpansionBreakdown (+13.60%, S=0.71)
- PanicAcceleration uses ROC(14) drop from >-2% to <-4% in 5 bars + ATR>avg + vol spike
- BTC needs FEWER conditions (3-4 max) and WIDER thresholds than ETH
- BTC stop: 2.0x ATR. Trail: wider (2.0x->1.5x->1.0x at 0R/1R/2.5R). Looser RSI: <50 (not <45).

WHEN INITIAL DATA IS ETH-USD-4h:
- ETH is the DEFAULT bear asset. Most proven winners are ETH-optimized.
- Use standard sweep defaults: vol>2.0x, RSI<45, trail 2.0x/1.5x/1.0x at 0R/0.8R/2.5R

========================================================================
 PERCENTILE INDICATOR GUIDELINES
========================================================================
Use percentile ranking for price-based momentum:
```python
def roc_percentile(close, roc_period=20, rank_window=100):
    close = pd.Series(close)
    roc = close.pct_change(roc_period)
    return roc.rolling(window=rank_window).rank(pct=True)

# Entry: roc_pct > 0.70 (top 30% momentum)
```

THRESHOLD GUIDELINES (strict self-gating for full-data):
- Single percentile filter: 0.70-0.80 threshold (top 20-30%)
- Dual percentile filters: 0.65-0.72 each (compensate for multiplication effect)
  - Example: ROC_pct > 0.68 AND RSI_pct > 0.65 (NOT 0.45-0.58!)
- MAXIMUM 2 percentile filters total (3+ = signal starvation or commission death)
- BETTER PATTERN: 1 percentile (0.70+) + 1-2 BOOLEAN filters (no multiplication effect)

Multi-timeframe momentum confirmation:
- Short-term: 10-bar ROC ranked in 50-bar window (> 0.65-0.75 percentile)
- Medium-term: 20-bar ROC ranked in 100-bar window (> 0.60-0.72 percentile)
- Combine both for high-conviction setups

SAFE TO RANK (price-based): ROC, RSI, ADX, ATR (for compression detection)

NEVER RANK (volume-based - causes signal starvation):
- Volume, OBV, MFI, AD
- Volume is lognormal with extreme outliers - percentile ranks are chaotic
- Always use volume as BOOLEAN: Volume > 1.5x rolling_mean

For BEAR strategies: use ROC BELOW percentile thresholds (bottom 20-30% = strong bear momentum).
Example: ROC percentile < 0.30 means bottom 30% = strong downtrend confirmation.

========================================================================
 VOLUME RULES
========================================================================
1. Volume as BOOLEAN only (never percentile rank):
   - Volume > 1.5-2.0x rolling 20-bar average
   - OBV[-1] > OBV[-2] (directional)
   - MFI > 60 or < 40 (absolute levels)

2. SINGLE volume filter per entry:
   - ONE "Volume[-1] > vol_avg[-1] * X.X" check
   - No extra volume momentum checks (volume[-1] > volume[-2])
   - No tightening beyond specification (idea says 2.0x, use 2.0x, NOT 2.5x)

3. Volume filter MUST be in the FINAL entry condition (before self.sell()):
   WRONG: setup = compressed and Volume > X; if setup: self.sell()
   CORRECT: if setup and Volume[-1] > vol_avg[-1] * 2.0: self.sell(size=size)

CANARY CHECK before generating code:
1. Find your self.sell() line (SHORT entry)
2. Does the if-statement contain a volume-based condition? This can be:
   - Raw volume check: "self.data.Volume[-1] > vol_avg[-1] * X.X" (most common)
   - OR a volume-flow indicator: OBV at N-bar low, MFI < threshold, CMF < threshold (these COUNT as volume confirmation)
3. If NO volume condition at all: ADD a raw volume check NOW
4. If YES: Is there MORE THAN ONE volume check? Remove extras (one volume signal is enough).
5. Does the if-statement contain "self.data.Close[-1] < self.sma200[-1]" (BEAR gate)?
6. If NO: ADD IT NOW - bear gate is MANDATORY!

# DEVANTSA_VOLUME_ENFORCEMENT_V3_APPLIED

========================================================================
 SQUEEZE/COMPRESSION -> BREAKDOWN STRATEGIES (TWO-STAGE LOGIC)
========================================================================
NEVER check compression AND expansion on the SAME bar - they are opposite states!
For SHORT strategies: compression then DOWNWARD expansion = short the breakdown.

CORRECT (time-separated, SHORT version):
```python
bear_ok = self.data.Close[-1] < self.sma200[-1]        # BEAR gate
compressed_recently = bb_width_pct[-2] < 0.35 or bb_width_pct[-3] < 0.35
breaking_down = roc[-1] < -0.03                          # Negative momentum expansion
volume_surge = self.data.Volume[-1] > 1.8 * vol_avg[-1]

if bear_ok and compressed_recently and breaking_down and volume_surge and not self.position:
    self.sell(size=size)  # SHORT the breakdown
```

Alternative - detect squeeze EXIT to downside:
```python
bear_ok = self.data.Close[-1] < self.sma200[-1]
was_squeezed = bb_width_pct[-2] < 0.25
now_expanding = bb_width_pct[-1] > 0.40
momentum_bearish = roc[-1] < -0.02

if bear_ok and was_squeezed and now_expanding and momentum_bearish:
    self.sell(size=size)  # SHORT entry
```

Compression thresholds: ATR_pct < 0.30-0.40 (bottom 30-40%).
Too tight (< 0.20-0.25) causes signal starvation.

If strategy name contains "Squeeze", "Compression", "Contraction", or "Flag":
YOU MUST use two-stage logic with time separation AND bear gate!

========================================================================
 RALLY FADE / REJECTION DETECTION RULES (FOR SHORTS)
========================================================================
Fade/rejection condition MUST be a SINGLE simple comparison:
- rsi > 60 (rally exhaustion in downtrend)
- close > ema21 (price rallied to resistance)
- close > sma20 * 0.98 (near resistance)

NO compound "AND" operators within the fade condition itself.
Maximum 4 total entry filters: bear gate + fade signal + turning down + volume.

MANDATORY FADE PATTERN #1 (EMA Rejection Short):
```python
bear_ok = self.data.Close[-1] < sma200[-1]            # BEAR gate
fade_ok = self.data.Close[-1] > ema21[-1] * 0.99      # Rallied near EMA resistance
turning_down = self.data.Close[-1] < self.data.Close[-2]  # Price turning down
volume_ok = self.data.Volume[-1] > 1.5 * vol_avg[-1]

if bear_ok and fade_ok and turning_down and volume_ok and not self.position:
    self.sell(size=size)  # SHORT entry
```

MANDATORY FADE PATTERN #2 (RSI Overbought Fade):
```python
bear_ok = self.data.Close[-1] < sma200[-1]            # BEAR gate
fade_ok = rsi[-1] > 60  # Rally exhaustion in downtrend
turning_down = rsi[-1] < rsi[-2]  # RSI turning down
volume_ok = self.data.Volume[-1] > 1.5 * vol_avg[-1]
...
If strategy name contains "Fade", "Rejection", "Rally", or "Exhaustion":
YOU MUST use one of the mandatory patterns above!
If your fade strategy generates <10 trades on 4h, your pattern is too strict.

========================================================================
 BANNED PATTERNS (CONSOLIDATED - SHORT STRATEGIES)
========================================================================
- self.buy() for entry (MUST use self.sell() for SHORT entries!)
- Entry WITHOUT price < SMA200 bear gate (will fire in bull markets and lose money)
- Tracking highest_since_entry (shorts track LOWEST_since_entry)
- Trail below price (shorts trail ABOVE price)
- Volume/OBV/MFI/AD percentile ranks (.rolling().rank(pct=True) on volume-based indicators)
- 3+ percentile filters combined (commission death OR signal starvation)
- Compression + momentum on same bar (logically impossible, generates 0 trades)
- bt.plot() (timeout in parallel processing)
- talib (not installed - use pandas/numpy)
- backtesting.lib.crossover() (import errors - use array indexing)
- .rank(pct=True) directly on self.data columns (system crash)
- Bare self.sell() without size parameter (full equity = catastrophic risk)
- Fixed % stops (always use ATR-based)
- Fixed TP without trailing stop (unless idea explicitly says "fixed TP")
- Max hold < 100 bars on pyramid/trailing strategies (use 200 bars for pyramid, 150 for simple trail)
- Printing on every bar in next() (only on trade entry/exit)
- Multiple volume conditions in entry (use exactly one)
- Pyramid add without profitability check (must check self.position.pl > 0 for units 2+)
- Fixed trail multiplier on pyramid strategies (must use adaptive: 2.0x/1.5x/1.5x at 0R/0.8R/2.5R)
- MAX_UNITS > 2 (standard is 2, proven higher Sharpe than 3 across 10 batches)
- MAX_UNITS < 1 (minimum 1 unit, 2 is default for pyramid strategies)
- Trade frequency > 100 on 4h data = strategy is churning, add stricter filters (target 30-80)
- Going LONG (self.buy) - this is a SHORT-ONLY bear strategy agent!
- SMA periods > 200 (wastes warmup data: SMA300 = 50 days lost, SMA200 = 33 days. Use SMA150-200 max)
- Divergence-based entries (hidden divergence, MACD divergence - CATASTROPHIC: -100% SOL in B10, all failed on crypto 5yr)
- Cascade/waterfall patterns (unreliable on crypto)
- Stochastic entries (StochasticRejection: all 3 assets negative in Batch 4)
- Bollinger Band walk entries (LowerBandWalk: all 3 assets negative, high churn 107-123 trades)
- Failed SMA200 reclaim entries (FailedReclaim: 2/3 assets negative in Batch 4)
- Wide bar breakdown entries without momentum (WideBarBreakdown: high churn, BTC/ETH negative)
- EMA ribbon entries (churns on 4h, too many signals in ranging markets)
- Wick rejection on 4h data (signal starvation - wicks too small relative to 4h bars)
- 7+ entry conditions (too selective, <20 trades over 5yr, statistically meaningless. MAX 4 conditions + bear gate)

========================================================================
 CODE STRUCTURE & EXECUTION BLOCK
========================================================================

Your code MUST have TWO parts:
PART 1: Strategy class (imports, helper functions, class with init/next)
PART 2: if __name__ == "__main__" block (MANDATORY!)

Include:
1. All necessary imports
2. Strategy class with indicators
3. Entry/exit logic with 3-4 conditions (trend filter + breakout/momentum + volume; stacked EMA bonus)
4. Risk management (1.0-1.2% per trade, ATR-based stops, 1.8x ATR initial stop for structural, 1.2x for momentum)
5. Starting cash: $1,000,000, commission: 0.001

DATA HANDLING:
1. Clean column names: data.columns = data.columns.str.strip().str.lower()
2. Drop unnamed columns: data = data.drop(columns=[col for col in data.columns if 'unnamed' in col.lower()])
3. Map to proper case: 'Open', 'High', 'Low', 'Close', 'Volume'

BACKTEST EXECUTION:
1. Run initial backtest with default parameters
2. Print full stats: stats = bt.run(); print(stats)
3. No optimization code needed
4. No plotting ever
...
if __name__ == "__main__":
    ...
    data = pd.read_csv('{DATA_PATH}\\\\{INITIAL_ASSET}-USD-4h.csv')
    data['datetime'] = pd.to_datetime(data['datetime'])
    data = data.set_index('datetime')
    data.columns = ['Open', 'High', 'Low', 'Close', 'Volume']

    bt = Backtest(data, YourStrategyClassName, cash=1_000_000, commission=0.001)
    stats = bt.run()
    ...
    data_dir = '{DATA_PATH}'
    csv_files = [f for f in os.listdir(data_dir) if f.endswith('.csv') and '4h' in f]
    ...
    for csv_file in csv_files:
        try:
            ...
            bt_test = Backtest(test_data, YourStrategyClassName, cash=1_000_000, commission=0.001)
    ...
    if results:
        ...
        os.makedirs('results_bear', exist_ok=True)
        results_csv_path = f'results_bear/YourStrategyName.csv'
...
========================================================================
 CRITICAL REQUIREMENTS - YOUR CODE WILL BE REJECTED IF YOU MISS ANY!
========================================================================

1. Replace 'YourStrategyClassName' with your actual strategy class name
2. Replace 'YourStrategyName' with the same name as a string
3. Include COMPLETE if __name__ == "__main__" block EXACTLY as shown above
4. Include multi-data testing section - IT IS MANDATORY!
5. Minimum 3 entry conditions: BEAR GATE (price < SMA200) + breakdown/rejection signal + volume
6. 1% risk per trade position sizing with size clamping: max(0.01, min(0.5, raw_size))
7. Use SHORT PYRAMID + ADAPTIVE TRAIL: self.sell(size=size), track lowest_since_entry, trail ABOVE price
8. Every strategy MUST use self.sell() for entries (NOT self.buy!) - these are SHORT strategies
9. Every strategy MUST include price < SMA200 as the primary BEAR gate

Use this data path: {DATA_PATH}\\{INITIAL_ASSET}-USD-4h.csv
The data format:
...
NO EMOJIS in print statements - Windows CP1252 crashes! Use plain ASCII only.
...
2. _Array Objects (NOT pandas Series):
   - .shift(), .rolling(), .iloc[], .values, .pct_change() will CRASH on _Array
   - Fix: Wrap in self.I() with pd.Series conversion inside
   - Example: self.I(lambda x: pd.Series(x).shift(1), self.data.Close)
   - If fixing one _Array issue, scan for ALL pandas methods in the code

3. Position Object:
   - NO .entry_price, .entry_bar, .sl, .tp attributes
   - Track manually as class variables (self.entry_price = None in init)
   - Available: .size, .pl, .pl_pct, .is_long, .is_short
   - For stop losses: use sl= parameter in buy/sell calls

4. Position Sizing:
   - Must be fraction (0 < size < 1) OR positive whole number (integer)
   - ALWAYS clamp: size = max(0.01, min(0.5, raw_size))
   - ALWAYS validate: if size > 0 and size < 1: self.sell(size=size)
   - Prevent division by zero in risk calculations: max(denominator, 0.001)

5. Broker API:
   - NEVER use self._broker.get_cash() or self._broker.get_value()
   - Use self.equity for current account value

6. No talib:
   - Replace talib imports with pandas/numpy equivalents
   - RSI: pd.Series(close).diff() based calculation
   - SMA: pd.Series(x).rolling(n).mean()
   - EMA: pd.Series(x).ewm(span=n).mean()
   - Always wrap in self.I()

7. No backtesting.lib:
   - Replace crossover(): use array indexing (signal[-2] < ref[-2] and signal[-1] > ref[-1])

8. No Trades (0 trades output):
   - Check self.sell() is actually called in entry condition (SHORT strategies use self.sell)
   - Check size parameter validity
   - Check entry conditions aren't too restrictive

Focus on fixing the specific error above. Scan for related issues of the same type.
...
Return the complete fixed code with proper debug prints (NO EMOJIS - plain ASCII only)!
ONLY SEND BACK CODE, NO OTHER TEXT.
"""
...
Max Drawdown: {current_dd}%
Trade Count: {current_trades}
Win Rate: {current_winrate}%
TARGET RETURN: {target_return}% (aspirational goal)

OPTIMIZATION ITERATION: {iteration} of {max_iterations}
{previous_attempts}

YOUR MISSION: Maximize COMPOSITE SCORE (60% Sharpe + 15% Return + 15% DD protection + 10% Win Rate).
SHARPE IS KING. Target Sharpe > 1.2. Batch 5 hit 1.09 - push past it. A smoother equity curve beats higher returns.
Sharpe improvement of 0.1 is MORE valuable than 5% return improvement.

PROP FIRM CONSTRAINT (Crypto Fund Trader): Max drawdown MUST stay under 8%.
If DD is already under -5%, do NOT sacrifice Sharpe to improve DD further.

CRITICAL SHARPE FLOOR RULE: If the current Sharpe is above 0.7, do NOT accept changes that drop
it below 0.6 even if return improves. PROTECT Sharpe at all costs.

TRADE COUNT WARNING: If trade count > 100, ADD stricter entry filters to reduce trades.
Sweet spot is 30-60 trades. Fewer high-quality trades = higher Sharpe.

BREAK-EVEN STOP: Ensure the strategy has break-even logic: once profit >= 1R, cap trailing stop
at entry_price. This converts winners to zero-risk trades and directly boosts Sharpe.

ITERATION-SPECIFIC FOCUS (you MUST try something DIFFERENT each iteration):
- Iteration 1: If Sharpe < 0.6, TIGHTEN stops (1.5x ATR for momentum, keep 1.8x for structural) and REDUCE max hold to 150 bars. Smoother equity curve first.
- Iteration 2: Adjust indicator lookback periods by 15-25%. Tighten R-thresholds (0.8R/2.5R for trail tightening).
- Iteration 3: ADD entry selectivity: require volume > 2.0x (not 1.5x) OR add momentum threshold. Fewer trades = higher quality.
- Iteration 4: INCREASE risk per unit to 1.5% if DD is under -5%. Bear strategies need to PROFIT. 1.5% is the MAX (prop firm safety).
- Iteration 5: Add one entry condition (stacked EMA, RSI band 55-65) OR tighten existing thresholds by 10-20%.
- Iteration 6: Combine the best changes from iterations 1-5. Focus on maximizing Sharpe while keeping return > 5%.

You MUST make at least 3 meaningful parameter changes. If you return identical code, the iteration is wasted.

IMPORTANT RULES (SHORT STRATEGIES):
...
- DO NOT remove SHORT pyramid logic! Keep self.units counter, MAX_UNITS, and profitability check.
- DO NOT remove adaptive trailing stop! Keep the R-multiple based trail tightening (2.0x/1.5x/1.5x at 0R/0.8R/2.5R).
- DO NOT remove the BEAR GATE (price < SMA200). You may add additional bear filters.
- DO NOT change entry from self.sell() to self.buy() - these are SHORT strategies!
- DO NOT change trailing stop to track highest (must track LOWEST for shorts).
- DO NOT change MAX_UNITS above 3 or below 1. Default is 2 (proven higher Sharpe than 3).
- DO NOT remove the profitability check for pyramid adds (self.position.pl > 0).
- DO NOT reduce position sizing below 1% risk per unit. 1% risk is the FLOOR, not a suggestion.
  If DD is already under -5%, INCREASE risk to 1.5% per unit to boost returns. 1.5% is the MAX.
- DO NOT reduce max hold below 100 bars on pyramid strategies.

Return the COMPLETE optimized code with plain text comments explaining what you changed!
ONLY SEND BACK CODE, NO OTHER TEXT.
"""
...
def detect_target_asset(text):
    """Detect target asset from idea text via [TARGET: XXX] tag."""
    text_upper = text.upper()
    if '[TARGET: SOL]' in text_upper or '[TARGET:SOL]' in text_upper:
        return 'SOL'
    elif '[TARGET: ETH]' in text_upper or '[TARGET:ETH]' in text_upper:
        return 'ETH'
...
def calculate_composite_score(stats: dict, thread_id: int) -> float:
    """
     PHASE 2: Multi-objective composite scoring (SHARPE-FIRST)

    Balances multiple objectives with Sharpe as primary driver:
    - 60% Sharpe Ratio (risk-adjusted returns - PRIMARY)
    - 15% Raw Returns (scaled to 0-1 range)
    - 15% Drawdown Protection (inverse of max drawdown)
    ...
    try:
        ...
        # Calculate weighted composite score (SHARPE-FIRST: 60/15/15/10)
        composite = (
            0.60 * sharpe_normalized +
            0.15 * return_normalized +
            0.15 * dd_normalized +
            0.10 * wr_normalized
        )
...
def log_stats_to_csv(strategy_name: str, thread_id: int, stats: dict, file_path: str, data_source: str = "BTC-USD-4h.csv") -> None:
    """
...
def parse_and_log_multi_data_results(strategy_name: str, thread_id: int, backtest_file_path: Path) -> None:
    ...
    try:
        ...
        results_dir = RBI_REGULAR_DIR / "results_bear"
...
def health_check_code(code: str, strategy_name: str, thread_id: int) -> tuple[bool, str, str]:
    ...
    try:
        ...
        # Check 7: Has sell logic (SHORT strategies use self.sell)
        if 'self.sell' not in code and 'self.position.close' not in code:
            return False, "No trading actions found (missing self.sell/close calls - bear strategies use self.sell for SHORT entry)", code
...
def qualify_strategy(stats: dict) -> bool:
    """
    Simple pass/fail strategy qualification for BEAR SHORT strategies.
    Returns True if strategy meets minimum quality thresholds.

    Thresholds (Batch 8 - raised after sweep optimization breakthrough):
    - Return > 5% (was 3% - sweep winners all >13%)
    - Sharpe > 0.80 (was 0.50 - 3 winners now S>1.0, raise the bar)
    - Max DD > -8% (tightened from -9% - sweep winners all <-3%)
    - Trades >= 20 (bear markets are shorter, 4h = fewer bars)
    """
    try:
...
def analyze_no_trades_issue(execution_result: dict) -> str:
    ...
    if 'ENTRY SIGNAL' in stdout and '# Trades                                    0' in stdout:
        return "Strategy is generating entry signals but self.sell() calls are not executing. This usually means: 1) Position sizing issues (size parameter invalid), 2) Insufficient cash/equity, 3) Logic preventing sell execution, or 4) Missing actual self.sell() call in the code. Bear strategies use self.sell() for SHORT entries."
...
def create_backtest(strategy, strategy_name, thread_id, target_asset='BTC'):
    """Backtest AI: Creates backtest implementation"""
    thread_print_status(thread_id, " BACKTEST", f"Creating code for {target_asset} with {BACKTEST_CONFIG['name']}...")
...
"""
    ...
    enhanced_backtest_prompt = enhanced_backtest_prompt.replace('{INITIAL_ASSET}', target_asset)
...
def optimize_strategy(backtest_code, current_return, target_return, strategy_name, thread_id, iteration=1, stats=None, previous_attempts=None, churning_warning=None):
    ...
    if churning_warning:
        prev_text += f"\nCRITICAL WARNING: {churning_warning}\nYou MUST add stricter entry filters to reduce trade count below 400. Increase volume threshold, add stacked EMA filter, or tighten momentum gate.\n"
...
def process_trading_idea_parallel(idea: str, thread_id: int) -> dict:
    ...
    try:
        ...
        # Detect target asset from idea text
        target_asset = detect_target_asset(idea)
        thread_print(f" Target asset: {target_asset}", thread_id, "cyan")
        ...
        # Phase 2: Backtest (initial data = target asset)
        backtest = create_backtest(strategy, strategy_name, thread_id, target_asset=target_asset)
...
def main(ideas_file_path=None, run_name=None, fulldata=True):
    ...
    if ideas_file_path:
        ...
        # Use the bear ideas file in RBI_Regular/
        IDEAS_FILE = RBI_REGULAR_DIR / "ideas_bear.txt"
